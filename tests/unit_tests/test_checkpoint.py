...
from torchtitan.components.checkpoint import AsyncSaveResponse, CheckpointManager
...
def fake_async_save(*args, **kwargs):
    ...
    if "async_stager" in kwargs:
        return AsyncSaveResponse(
            staging_completion=DummyFuture(),
            upload_completion=DummyFuture(),
        )
    else:
        return DummyFuture()
...
class TestCheckpointManager(unittest.TestCase):
    ...
    def test_save_and_purge_keeps_last_k_checkpoints(
        ...
        while True:
            exist = sorted(os.listdir(manager.folder))
        ...
        self.assertListEqual(sorted(os.listdir(manager.folder)), ["step-2", "step-3"])
        calls = [c.kwargs.get("checkpoint_id") for c in mock_save.call_args_list]
        expected = [os.path.join(manager.folder, f"step-{i}") for i in (1, 2, 3)]
        self.assertListEqual(calls, expected)
        sd = torch.load(
            os.path.join(manager.folder, "step-3", "state_dict.pt"),
            weights_only=False,
        )
    ...
    def test_nonzero_rank_does_not_purge_or_save(self, mock_load, mock_save, mock_rank):
        ...
        self.assertListEqual(
            sorted(os.listdir(manager.folder)), ["step-1", "step-2", "step-3"]
        )
    ...
    def test_last_save_model_only_and_initial_load_model_only(
        ...
        path1 = os.path.join(manager1.folder, "step-1")
        ...
        step2_dir = os.path.join(manager2.folder, "step-2")
    ...
    def test_non_persist_buffer_not_saved(self, mock_save, mock_rank):
        ...
        manager.save(curr_step=1)
        self.assertEqual(mock_save.call_count, 1)
        checkpoint_path = os.path.join(manager.folder, "step-1", "state_dict.pt")
