    ...
    def setUp(self):
        def build_synthetic_dataset():
            return Dataset.from_dict(
                {
                    "text": [
                        f"synthetic checkpoint sample {idx} " * 128
                        for idx in range(32)
                    ]
                }
            )

        DATASETS["checkpoint_test"] = DatasetConfig(
            path="unused",
            loader=lambda _: build_synthetic_dataset(),
            sample_processor=lambda sample: sample["text"],
        )

    def tearDown(self):
        del DATASETS["checkpoint_test"]

    def test_synthetic_arrow_resumption(self):
        for dataset_name in ["checkpoint_test"]:
