...
class TestJobConfig(unittest.TestCase):
    ...
    def test_optimizer_module_lr_overrides_parsing(self):
        with tempfile.NamedTemporaryFile() as fp:
            with open(fp.name, "wb") as f:
                tomli_w.dump(
                    {
        ...
        assert len(config.optimizer.module_lr_overrides) == 1
        override = config.optimizer.module_lr_overrides[0]
        assert override.name == "attention"
        assert override.target_globs == ["layers.*.attention.*"]
        assert override.lr is None
        assert override.lr_multiplier == 3.0

    def test_peft_config_parsing(self):
        config_manager = ConfigManager()
        config = config_manager.parse_args(
            [
                "--model.converters",
        ...
        assert config.model.converters == ["peft"]
        assert config.peft.target_globs == ["layers.*.attention.wq", "output"]
        assert config.peft.exclude_globs == ["output"]
        assert config.peft.trainable_base_globs == ["tok_embeddings.weight"]
        assert config.peft.lora.rank == 8
        assert config.peft.lora.alpha == 16

    def test_training_data_visualization_config_parsing(self):
        with tempfile.NamedTemporaryFile() as fp:
            with open(fp.name, "wb") as f:
                tomli_w.dump(
                    {
                        "training_data_visualization": {
                            ...
                            "wandb_fixed_html_url": "https://example.com/viz.html",
                        }
                    },
                    f,
                )
        ...
        assert config.training_data_visualization.enable is True
        assert config.training_data_visualization.freq == 25
        assert config.training_data_visualization.num_samples == 3
        assert config.training_data_visualization.log_to_wandb is True
        assert config.training_data_visualization.async_worker is True
        assert config.training_data_visualization.max_pending_jobs == 4
        assert config.training_data_visualization.upload_to_s3 is False
        assert config.training_data_visualization.s3_bucket == "custom-bucket"
        assert config.training_data_visualization.s3_prefix == "custom/prefix"
        assert config.training_data_visualization.s3_region == "us-east-1"
        assert config.training_data_visualization.wandb_log_url_only is False
        assert (
            config.training_data_visualization.wandb_fixed_html_url
    ...
    def test_job_config_invalid_field(self):
        with self.assertRaisesRegex(
            ...
            r"Run `python -m torchtitan\.train --help` to read all valid fields\.",
