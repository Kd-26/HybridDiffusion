...
class TikTokenTokenizerWrapper(BaseTokenizer):
    """
    A wrapper for TikToken-based tokenizers loaded via transformers AutoTokenizer.
    
    This wrapper makes TikTokenTokenizer (used by Kimi models) compatible with 
    the BaseTokenizer interface used in torchtitan.
    """
    ...
    def __init__(self, tokenizer):
        ...
        self.bos_id = tokenizer.bos_token_id if hasattr(tokenizer, 'bos_token_id') else None
        self.eos_id = tokenizer.eos_token_id if hasattr(tokenizer, 'eos_token_id') else None
        self.pad_id = tokenizer.pad_token_id if hasattr(tokenizer, 'pad_token_id') else None
        
        logger.info(f"Loaded TikToken tokenizer: vocab_size={len(tokenizer)}, "
    ...
    def encode(self, text: str, bos: bool = False, eos: bool = False) -> list[int]:
        ...
        Args:
            ...
            bos: Whether to add BOS token at the beginning
            eos: Whether to add EOS token at the end
            
        Returns:
            List of token IDs
        """
        # Use the tokenizer's encode method (not encode_plus)
        # transformers tokenizers typically don't add special tokens by default
...
class HuggingFaceTokenizer(BaseTokenizer):
    ...
    def __init__(
        ...
        # Initialize special token attributes (frequently used)
        self.bos_id = None
        ...
        if not os.path.exists(tokenizer_path):
            if "assets/tokenizer" in tokenizer_path:
                raise FileNotFoundError(
                    ...
                    "Download tokenizer assets with ./scripts/download_hf_assets.py."
                )
            else:
                raise FileNotFoundError(
    ...
    def _process_special_token(
        ...
        """
        # Get reference special tokens from config for comparison
        config_bos_token = (
        ...
        config_mask_token = (
            self._get_token_from_config(self.config, "mask_token")
            if self.config
            else None
        )
        ...
        # Store special tokens as class attributes if they match
        if token_str == config_bos_token:
            self.bos_token = token_str  # pyrefly: ignore[bad-assignment]
            self.bos_id = resolved_id  # pyrefly: ignore[bad-assignment]
        if token_str == config_eos_token:
            self.eos_token = token_str  # pyrefly: ignore[bad-assignment]
            self.eos_id = resolved_id
        if token_str == config_pad_token:
            self.pad_token = token_str  # pyrefly: ignore[bad-assignment]
            self.pad_id = resolved_id
        if token_str == config_mask_token:
            self.mask_token = token_str  # pyrefly: ignore[bad-assignment]
            self.mask_id = resolved_id
    ...
    def _infer_special_tokens(self):
        ...
        if added_tokens_to_add:
            ...
            # Update token IDs after adding to tokenizer (in case they changed)
            if self.bos_token:
...
def build_hf_tokenizer(
    ...
    """
    # Try to use hf_transformers.tokenizer_path first, fall back to hf_assets_path
    tokenizer_path = job_config.model.hf_assets_path
    if hasattr(job_config, 'hf_transformers') and job_config.hf_transformers.tokenizer_path:
        tokenizer_path = job_config.hf_transformers.tokenizer_path
    
    # Cache S3 path to local if needed
    tokenizer_path = cache_s3_path_if_needed(tokenizer_path, category="weights")
    
    # Convert relative path to absolute path if needed
    if tokenizer_path and not os.path.isabs(tokenizer_path):
        # Resolve relative path from the DLLM directory (parent of torchtitan)
        # __file__ is in torchtitan/components/, so go up 2 levels to get DLLM dir
        dllm_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        tokenizer_path = os.path.join(dllm_dir, tokenizer_path)
