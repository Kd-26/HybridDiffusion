...
class ParallelAwareDataloader(StatefulDataLoader, BaseDataLoader):
    ...
    def __iter__(self):
        """Return the stateful iterator.

        Declaring this explicitly satisfies ``BaseDataLoader``'s abstract
        contract across torchdata versions where the inherited DataLoader
        implementation is not recognized by ``ABCMeta``.
        """
