...
class ConfigManager:
    ...
    def parse_args(self, args: list[str] = sys.argv[1:]) -> JobConfig:
        toml_values, config_file_path = self._maybe_load_toml(args)
        ...
        # Store config file path in job config if it was loaded from toml
        if config_file_path:
            self.config.job.config_file = config_file_path

        self._expand_environment_paths()
        self._validate_config()

        return self.config

    def _expand_environment_paths(self) -> None:
        """Expand explicit environment variables in model and dataset paths.

        Existing paths without ``$VARNAME``/``${VARNAME}`` markers are left
        unchanged. This keeps historical URI and Hugging Face dataset behavior
        intact while allowing public configs to use persistent-storage roots.
        """
        ...
        self.config.model.hf_assets_path = expand(
            self.config.model.hf_assets_path
        )
        self.config.training.dataset_path = expand(
            self.config.training.dataset_path
        )
        self.config.validation.dataset_path = expand(
            self.config.validation.dataset_path
        )
        for entry in self.config.training.dataset_mix:
            entry.path = expand(entry.path)

    def _maybe_load_toml(self, args: list[str]) -> tuple[dict[str, Any] | None, str | None]:
        # 1. Check CLI
        ...
        try:
            with open(file_path, "rb") as f:
                return tomllib.load(f), file_path
    ...
    def _dict_to_dataclass(self, cls, data: dict[str, Any]) -> Any:
        ...
        if invalid_fields := set(data) - valid_fields:
            raise ValueError(
                ...
                "Run `python -m torchtitan.train --help` to read all valid fields."
            )
        ...
        for f in fields(cls):
            if f.name in data:
                ...
                if is_dataclass(f.type) and isinstance(value, dict):
                    ...
                    inner = self._get_list_inner_dataclass(f.type)
                    if inner is not None:
                        result[f.name] = [
                            self._dict_to_dataclass(inner, item)
                            for item in value
                        ]
                    else:
                        result[f.name] = value
    ...
    @staticmethod
    def _get_list_inner_dataclass(type_hint):
        """Return the inner dataclass type if *type_hint* is ``list[SomeDataclass]``."""
    ...
    def _validate_config(self) -> None:
        ...
        # Skip validation for S3 paths - they will be cached by individual components
        # (tokenizer, checkpoint, etc.) after distributed initialization when rank 0 can be determined
        if self.config.model.hf_assets_path.startswith("s3://"):
            logger.info(f"S3 path detected for hf_assets_path: {self.config.model.hf_assets_path}")
            logger.info("Path validation and caching will be performed by components after distributed initialization")
        elif not os.path.exists(self.config.model.hf_assets_path):
            logger.warning(
