...
class Job:
    ...
    trial_name: str = ""
    """
    A human-readable name for this training trial / experiment.
    When set, it is appended to the config basename to form the run
    directory (e.g. ``{config_basename}_{trial_name}``).
    If None, only the config basename is used.
    """

    run_name: str = ""
    """
    Explicit run directory name. When set, this overrides the auto-generated
    ``{config_basename}_{trial_name}`` logic entirely for checkpoint paths,
    S3 upload paths, and metrics directories. Useful for multi-stage pipelines
    where the checkpoint directory should be named after the pipeline
    configuration rather than the per-stage TOML file.
    """
...
class Metrics:
    ...
    """WandB server URL for an optional self-hosted instance"""

    wandb_mode: str = "online"
    """WandB mode: online, offline, or disabled"""

    wandb_name: str | None = None
    """WandB run name. If None, auto-generated from config file name."""
...
@dataclass
class OptimizerModuleLROverride:
    name: str = ""
    """Optional label used in logs and error messages."""

    target_globs: list[str] = field(default_factory=list)
    """
    Parameter FQN glob patterns to match against, for example
    ``layers.*.attention.*`` or ``output.weight``.
    """

    lr: float | None = None
    """Absolute learning rate for matched parameters."""

    lr_multiplier: float | None = None
    """Multiplier applied to ``optimizer.lr`` for matched parameters."""
...
class Optimizer:
    ...
    """
    FlashOptim master weight precision. Only used when name is FlashAdamW/FlashAdam.
    24 = 8-bit correction (default, best memory savings),
    32 = 16-bit correction (full fp32 semantics),
    None = no correction (params stay at native precision).
    """

    compress_state_dict: bool = False
    """
    FlashOptim: quantize optimizer states to int8 in checkpoints for ~50%
    smaller checkpoint files.  Only used with FlashAdamW/FlashAdam.
    """

    module_lr_overrides: list[OptimizerModuleLROverride] = field(
        default_factory=list
    )
    """
    Optional per-module learning-rate overrides implemented as param groups
    within the same optimizer. Unmatched parameters keep ``optimizer.lr``.
    Each entry matches parameter FQNs using glob syntax.
    """
...
class DatasetMixEntry:
    """One component of a multi-dataset mix."""

    name: str = ""
    """Dataset name (must exist in the DATASETS registry)."""

    path: str = ""
    """
    Override path for this dataset.  S3 paths are automatically cached
    to local SSD.
    """

    weight: float = 1.0
    """
    Sampling weight (unnormalized).  The dataloader normalizes all
    weights to sum to 1.
    """
...
class Training:
    ...
    steps: int | None = None
    """How many train steps to run (calculated from tokens if not provided)"""

    tokens: int | None = None
    """Total number of tokens to train on (will calculate steps automatically)"""
    ...
    """
    Whether to shift labels for next token prediction.
    Set to True for standard language modeling (input=x[:-1], label=x[1:]).
    Set to False for diffusion LLM or other models that don't use next token prediction.
    """

    dataset_mix: list[DatasetMixEntry] = field(default_factory=list)
    """
    List of dataset entries for multi-dataset mixing.  When non-empty,
    overrides the single ``dataset`` / ``dataset_path`` fields.

    Each entry is a ``DatasetMixEntry`` with ``name``, ``path``, and
    ``weight``.  In TOML use the array-of-tables syntax::

        [[training.dataset_mix]]
        name   = "nemotron_post_training_v2_sft"
        path   = "${DATA_ROOT}/Nemotron-Post-Training-Dataset-v2-arrow"
        weight = 0.5

        [[training.dataset_mix]]
        name   = "llama_nemotron_sft"
        path   = "${DATA_ROOT}/Llama-Nemotron-Post-Training-Dataset-arrow/SFT"
        weight = 0.5
    """
...
class Checkpoint:
    ...
    initial_ckpt_load_plan: str | None = None
    """
    Selective loading plan for the initial checkpoint. Specifies which checkpoint
    components to load into which memory targets. If a component is not present
    in the checkpoint, it will be skipped without error.

    Format: comma-separated 'ckpt_source:mem_target' pairs.
    Example: 'ckpt_model:mem_model,ckpt_optimizer:mem_optimizer'

    Valid sources: ckpt_model, ckpt_optimizer, ckpt_lr_scheduler (or ckpt_scheduler),
                   ckpt_train_state (or ckpt_step), ckpt_dataloader
    Valid targets: mem_model, mem_optimizer, mem_lr_scheduler (or mem_scheduler),
                   mem_train_state (or mem_step), mem_dataloader

    When set, this overrides initial_load_model_only. Components not listed in the
    plan will not be loaded. Components listed but missing from the checkpoint or
    not present in the current training state are silently skipped.

    If None, the default behavior (controlled by initial_load_model_only) is used.
    """
    ...
    """
    Base S3 path to upload checkpoints automatically after saving.
    If specified, checkpoints will be uploaded to this S3 location after each save.
    The user name and config name will be automatically appended to the path.
    
    Format: s3://bucket/path/to/base/
    Actual upload path: s3://bucket/path/to/base/{user_name}/{config_name}/
    
    User name is obtained from ``USER`` and falls back to ``"default"``.
    
    Config name is extracted from the config file basename (e.g., "deepseek_moe_16b" from "deepseek_moe_16b.toml")
    
    If None, no S3 upload will be performed.
    """
...
class PeftLora:
    ...
    dropout: float = 0.0
    """Dropout applied on the adapter branch input."""

    scaling: Literal["lora", "rs_lora"] = "rs_lora"
    """LoRA scaling strategy."""

    init: Literal["zero_start"] = "zero_start"
    """LoRA initialization strategy."""


@dataclass
class Peft:
    method: Literal["lora"] | str = "lora"
    """PEFT method name."""

    freeze_base_model: bool = True
    """Whether to freeze all base parameters by default."""

    target_globs: list[str] = field(default_factory=list)
    """Glob patterns selecting which module FQNs receive adapters."""

    exclude_globs: list[str] = field(default_factory=list)
    """Glob patterns excluded from adapter application after target matching."""

    trainable_base_globs: list[str] = field(default_factory=list)
    """Glob patterns selecting base-model parameter FQNs to keep trainable."""

    strict: bool = True
    """Whether invalid or empty PEFT selections should raise errors."""

    lora: PeftLora = field(default_factory=PeftLora)
    """LoRA-specific configuration."""

    adapters: list[str] = field(default_factory=list)
    """Named adapters to create. Empty list = single 'default' adapter (v1 compatible)."""

    active_adapter: str = "default"
    """Which adapter is initially active for forward computation."""

    trainable_adapters: list[str] = field(default_factory=list)
    """Which adapters have requires_grad=True. Empty list = all adapters trainable."""
...
class TrainingDataVisualization:
    enable: bool = False
    """Enable periodic training data visualization"""

    freq: int = 1000
    """Frequency of logging training data samples (in steps)"""

    num_samples: int = 10
    """Number of training data samples to log per HTML (clamped to at most 10)"""

    log_to_wandb: bool = False
    """Whether to log samples to wandb"""

    async_worker: bool = True
    """Render HTML asynchronously from a background worker thread"""

    max_pending_jobs: int = 2
    """Maximum queued visualization render jobs before dropping new ones"""

    upload_to_s3: bool = False
    """Upload rendered HTML to S3 and prefer logging a stable URL to wandb"""

    s3_bucket: str = ""
    """S3 bucket for rendered visualization HTML"""

    s3_prefix: str = "training_data_viz"
    """S3 key prefix for rendered visualization HTML"""

    s3_region: str = "us-west-2"
    """AWS region used to construct stable S3 URLs"""

    wandb_log_url_only: bool = False
    """When true, log only the stable HTML URL to wandb instead of embedding full HTML"""

    wandb_fixed_html_url: str | None = None
    """Optional fixed HTML URL to log to wandb for every visualization step"""


@dataclass
class DLLM:
    """Configuration for Diffusion Language Model (DLLM) training."""

    block_size: int = 16
    """Block size for diffusion masking (1 = token-level, higher = block-level)"""

    block_train_conv_method: Literal["auto", "fla_batched", "twostream"] = "auto"
    """BlockShortConv block-train dispatch method."""

    use_flex_attention: bool = True
    """Use FlexAttention with BlockMask (faster) vs SDPA with 4D mask"""

    complementary_mask: bool = True
    """Generate complementary masks to double effective batch size (B -> 2B)"""

    antithetic_sampling: bool = True
    """Use permuted stratified block-noise sampling for variance reduction"""

    mask_token_id: int = -1
    """Token ID for diffusion noise/mask (-1 = vocab_size-1 for backward compat).
    New experiments should set to 151669 (<|MASK|>)."""

    pad_token_id: int = -1
    """Token ID for padding unused positions (-1 = same as mask_token_id for backward compat).
    New experiments should set to 151643 (<|endoftext|>)."""

    sft_label_mode: Literal["response", "full", "full_content"] = "response"
    """Which ChatML token spans to supervise for DLLM SFT.
    ``response`` keeps current behavior (assistant-only supervision).
    ``full`` trains on all non-padding ChatML tokens.
    ``full_content`` trains on content tokens for all roles, but not headers/end markers."""

    causal_x0: bool = False
    """x_0 (clean) tokens use token-level causal attention instead of block-causal."""

    ar_loss_weight: float = 0.0
    """Weight for AR NTP loss on x_0 outputs. 0.0 = disabled.
    Total loss = (diff_loss + ar_loss_weight * ar_loss) / (1 + ar_loss_weight)."""

    use_fused_ce: bool = False
    """Use liger-kernel fused linear cross-entropy (hidden->loss, no logits materialization)."""

    # --- Diffusion recipe flags ---
    all_masked: bool = False
    """100% masking rate — all tokens replaced with MASK. Disables complementary_mask."""

    causal_xt: bool = False
    """x_t (noisy) tokens use causal attention within blocks instead of bidirectional attention."""

    logit_shift: bool = False
    """Dream-style label shift: position i predicts token i+1 (not token i)."""

    loss_auto_balance: bool = False
    """Dynamic delta = L_mask.detach() / L_clean.detach() replaces static ar_loss_weight."""

    loss_normalize: bool = True
    """Normalize combined loss by (1+ar_weight). False uses L + alpha*L_clean."""

    enable_packing: bool = False
    """Pack multiple SFT samples into one sequence with per-document attention isolation."""

    enable_weight_tying: bool = True
    """Tie tok_embeddings and output weights. Set False to untie (e.g., for diffusion models)."""
...
class JobConfig:
    ...
    training_data_visualization: TrainingDataVisualization = field(default_factory=TrainingDataVisualization)
    debug: Debug = field(default_factory=Debug)
    dllm: DLLM = field(default_factory=DLLM)
