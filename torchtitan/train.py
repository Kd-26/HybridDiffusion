...
class Trainer(torch.distributed.checkpoint.stateful.Stateful):
    ...
    training_data_logger: TrainingDataLogger | None
    ...
    def __init__(self, job_config: JobConfig):
        ...
        # Calculate steps from tokens if tokens is provided
        if job_config.training.tokens is not None:
            if job_config.training.steps is not None:
                logger.warning(
                    f"Both tokens ({job_config.training.tokens:,}) and steps "
                    f"({job_config.training.steps:,}) are specified. Using tokens to calculate steps."
                )
            tokens_per_step = global_batch_size * job_config.training.seq_len
            calculated_steps = job_config.training.tokens // tokens_per_step
            logger.info(
                f"Calculating training steps from tokens: "
                f"{job_config.training.tokens:,} tokens / "
                f"({global_batch_size} batch_size × {job_config.training.seq_len} seq_len) = "
                f"{calculated_steps:,} steps"
            )
            job_config.training.steps = calculated_steps
        elif job_config.training.steps is None:
            raise ValueError(
                "Either training.tokens or training.steps must be specified in the config"
            )
        ...
        model_converters.post_model_init(self.model_parts)
        ...
        self.checkpointer = CheckpointManager(
            ...
            config_file_path=job_config.job.config_file,
        ...
        # Build training data logger if visualization is enabled
        if job_config.training_data_visualization.enable and self.tokenizer is not None:
            self.training_data_logger = TrainingDataLogger(
                job_config=job_config,
                tokenizer=self.tokenizer,
            )
            logger.info("Training data logger initialized for periodic visualization")
        else:
    ...
    def post_dataloading_process(
        ...
        if self.job_config.model.name in {
            "qwen3_5_dllm",
            "qwen3_5_dllm_sft",
            "qwen3_5_hybrid_diffusion_sft",
        }:
            extra_kwargs["block_train"] = True
        ...
        if parallel_dims.pp_enabled:
            ...
            with self.train_context(optional_context_parallel_ctx):
                ...
                with self.maybe_enable_amp:
                    ...
                    if capture_viz:
                        viz_payload = self._extract_training_viz_payload(pred)
                    loss = self.loss_fn(pred, labels)
                # need to free pred before bwd to avoid peaking memory
                del pred
                loss.backward()

        return loss, viz_payload

    def _extract_training_viz_payload(
        self, pred: Any
    ) -> dict[str, Any] | None:
        ...
        if isinstance(masked_indices, torch.Tensor):
            payload["masked_indices"] = masked_indices.detach().cpu()

        layout = getattr(getattr(self.job_config, "dllm", None), "layout", None)
        if isinstance(layout, str) and layout:
            payload["layout"] = layout
        elif "masked_indices" in payload:
            payload["layout"] = "x0_xt_doubled"
    ...
    def train_step(
        ...
        accumulated_losses = []
        should_capture_viz = (
            self.training_data_logger is not None
            and self.training_data_logger.should_log(self.step)
        )
        last_viz_payload = None
        # If data runs out during gradient accumulation, that
        # entire step will not be executed.
        for _microbatch in range(self.gradient_accumulation_steps):
            # pyrefly: ignore [no-matching-overload]
            input_dict, labels = next(data_iterator)
            capture_this_microbatch = should_capture_viz and (
                _microbatch == self.gradient_accumulation_steps - 1
            )
            loss, viz_payload = self.forward_backward_step(
                input_dict,
                labels,
                capture_viz=capture_this_microbatch,
            )
            accumulated_losses.append(loss.detach())
            if capture_this_microbatch:
                last_viz_payload = viz_payload

        # Save last batch for training data visualization
        self._last_input_dict = input_dict
        self._last_labels = labels
        self._last_viz_payload = last_viz_payload
        ...
        # Collect auxiliary metrics (MoE, etc.) if available
        for model_part in self.model_parts:
            if hasattr(model_part, "get_aux_metrics"):
                extra_metrics.update(model_part.get_aux_metrics())
                break
    ...
    def train(self):
        ...
        with (
            ...
            while self.should_continue_training():
                ...
                # Log training data samples if enabled
                if (
                    self.training_data_logger is not None
                    and self.training_data_logger.should_log(self.step)
                ):
                    logger.info(f"Logging training data samples at step {self.step}")
                    # Get current batch data from last train_step
                    if hasattr(self, '_last_input_dict') and hasattr(self, '_last_labels'):
                        input_ids = self._last_input_dict.get('input', self._last_input_dict.get('input_ids'))
                        if input_ids is not None:
                            samples = self.training_data_logger.log_batch_samples(
                                input_ids=input_ids,
                                labels=self._last_labels,
                                step=self.step,
                                viz_payload=getattr(self, "_last_viz_payload", None),
                            )
                            if samples and self.job_config.training_data_visualization.log_to_wandb:
                                self.metrics_processor.log_training_data_samples(samples, self.step)
    ...
    def state_dict(self) -> dict[str, Any]:
        sd = {
            ...
            # Keep the DCP schema stable before W&B has initialized. DCP loads
            # only keys declared by the destination state dict, so omitting
            # this key would discard a run ID that exists in the checkpoint.
            "wandb_run_id": "",
        }
        # Save wandb run_id so we can resume the same run on a new machine
        wandb_run_id = self.metrics_processor.get_wandb_run_id()
        if wandb_run_id:
            sd["wandb_run_id"] = wandb_run_id
        return sd

    def load_state_dict(self, state_dict: dict[str, Any]):
        self.step = state_dict["step"]
        self.ntokens_seen = state_dict["ntokens_seen"]
        # Resume wandb with the saved run_id
        wandb_run_id = state_dict.get("wandb_run_id")
        if wandb_run_id:
            self.metrics_processor.resume_wandb(wandb_run_id)
            logger.info(f"Resumed WandB run: {wandb_run_id}")

    def close(self) -> None:
        if hasattr(self, "checkpointer") and self.checkpointer:
            self.checkpointer.close()
        if hasattr(self, "training_data_logger") and self.training_data_logger:
            self.training_data_logger.close()
        if hasattr(self, "metrics_processor") and self.metrics_processor:
            if hasattr(self.metrics_processor, "close"):
                self.metrics_processor.close()
            elif hasattr(self.metrics_processor, "logger") and hasattr(self.metrics_processor.logger, "close"):
                self.metrics_processor.logger.close()
