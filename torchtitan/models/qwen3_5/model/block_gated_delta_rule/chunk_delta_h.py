...
# Local copy of fla.ops.common.chunk_delta_h with grid dimension fix.
...
def chunk_gated_delta_rule_fwd_kernel_h_blockdim64(
    ...
    for i_t in range(NT):
        ...
        p_w = tl.make_block_ptr(w, (T_chunk, K), (H*K, 1), (i_t * CHUNK_SIZE, 0), (BT, 64), (1, 0))
        b_w = tl.load(p_w, boundary_check=(0, 1))
        b_v = tl.dot(b_w, b_h1.to(b_w.dtype))
        if K > 64:
            p_w = tl.make_block_ptr(w, (T_chunk, K), (H*K, 1), (i_t * CHUNK_SIZE, 64), (BT, 64), (1, 0))
            b_w = tl.load(p_w, boundary_check=(0, 1))
            b_v += tl.dot(b_w, b_h2.to(b_w.dtype))
        if K > 128:
            p_w = tl.make_block_ptr(w, (T_chunk, K), (H*K, 1), (i_t * CHUNK_SIZE, 128), (BT, 64), (1, 0))
            b_w = tl.load(p_w, boundary_check=(0, 1))
            b_v += tl.dot(b_w, b_h3.to(b_w.dtype))
        if K > 192:
            p_w = tl.make_block_ptr(w, (T_chunk, K), (H*K, 1), (i_t * CHUNK_SIZE, 192), (BT, 64), (1, 0))
            b_w = tl.load(p_w, boundary_check=(0, 1))
        ...
        if SAVE_NEW_VALUE:
            p_v = tl.make_block_ptr(v_new, (T_chunk, V), (H*V, 1), (i_t * CHUNK_SIZE, i_v * BV), (BT, BV), (1, 0))
            tl.store(p_v, b_v.to(p_v.dtype.element_ty), boundary_check=(0, 1))

        last_idx = min((i_t + 1) * CHUNK_SIZE, T) - 1
        if USE_G:
            m_t = (i_t * CHUNK_SIZE + tl.arange(0, BT)) < T
            b_g_last = tl.load(g + (bos * H + last_idx * H + i_h).to(tl.int64)).to(tl.float32)
            p_g = tl.make_block_ptr(g + (bos * H + i_h).to(tl.int64), (T_chunk,), (H,), (i_t * CHUNK_SIZE,), (BT,), (0,))
        ...
        p_k = tl.make_block_ptr(k, (K, T_chunk), (1, H*K), (0, i_t * CHUNK_SIZE), (64, BT), (0, 1))
        b_k = tl.load(p_k, boundary_check=(0, 1))
        b_h1 += tl.dot(b_k, b_v)
        if K > 64:
            p_k = tl.make_block_ptr(k, (K, T_chunk), (1, H*K), (64, i_t * CHUNK_SIZE), (64, BT), (0, 1))
            b_k = tl.load(p_k, boundary_check=(0, 1))
            b_h2 += tl.dot(b_k, b_v)
        if K > 128:
            p_k = tl.make_block_ptr(k, (K, T_chunk), (1, H*K), (128, i_t * CHUNK_SIZE), (64, BT), (0, 1))
            b_k = tl.load(p_k, boundary_check=(0, 1))
            b_h3 += tl.dot(b_k, b_v)
        if K > 192:
            p_k = tl.make_block_ptr(k, (K, T_chunk), (1, H*K), (192, i_t * CHUNK_SIZE), (64, BT), (0, 1))
            b_k = tl.load(p_k, boundary_check=(0, 1))
            b_h4 += tl.dot(b_k, b_v)
...
@triton.heuristics({
    ...
    'USE_DH_INJECT': lambda args: args['dh_inject'] is not None,
    'IS_VARLEN': lambda args: args['cu_seqlens'] is not None,
})
@triton.autotune(
    configs=[
...
def chunk_gated_delta_rule_bwd_kernel_dhu_blockdim64(
    ...
    for i_t in range(NT - 1, -1, -1):
        ...
        if USE_DH_INJECT:
            p_di1 = tl.make_block_ptr(dh_inject + i_t_int64*H*K*V, (K, V), (V, 1), (0, i_v * BV), (64, BV), (1, 0))
            b_dh1 += tl.load(p_di1, boundary_check=(0, 1))
            if K > 64:
                p_di2 = tl.make_block_ptr(dh_inject + i_t_int64*H*K*V, (K, V), (V, 1), (64, i_v * BV), (64, BV), (1, 0))
                b_dh2 += tl.load(p_di2, boundary_check=(0, 1))
            if K > 128:
                p_di3 = tl.make_block_ptr(dh_inject + i_t_int64*H*K*V, (K, V), (V, 1), (128, i_v * BV), (64, BV), (1, 0))
                b_dh3 += tl.load(p_di3, boundary_check=(0, 1))
            if K > 192:
                p_di4 = tl.make_block_ptr(dh_inject + i_t_int64*H*K*V, (K, V), (V, 1), (192, i_v * BV), (64, BV), (1, 0))
                b_dh4 += tl.load(p_di4, boundary_check=(0, 1))
        ...
        p_dv2 = tl.make_block_ptr(dv2, (T_chunk, V), (H*V, 1), (i_t * CHUNK_SIZE, i_v * BV), (BT, BV), (1, 0))
        p_do = tl.make_block_ptr(do, (T_chunk, V), (H*V, 1), (i_t * CHUNK_SIZE, i_v * BV), (BT, BV), (1, 0))

        b_do = tl.load(p_do, boundary_check=(0, 1))

        # Update dv
        p_k = tl.make_block_ptr(k, (T_chunk, K), (H*K, 1), (i_t * CHUNK_SIZE, 0), (BT, 64), (1, 0))
        b_k = tl.load(p_k, boundary_check=(0, 1))
        if USE_GK:
            o_k1 = tl.arange(0, 64)
        ...
        if K > 64:
            p_k = tl.make_block_ptr(k, (T_chunk, K), (H*K, 1), (i_t * CHUNK_SIZE, 64), (BT, 64), (1, 0))
            b_k = tl.load(p_k, boundary_check=(0, 1))
            if USE_GK:
                o_k2 = 64 + o_k1
        ...
        if K > 128:
            p_k = tl.make_block_ptr(k, (T_chunk, K), (H*K, 1), (i_t * CHUNK_SIZE, 128), (BT, 64), (1, 0))
            b_k = tl.load(p_k, boundary_check=(0, 1))
            if USE_GK:
                o_k3 = 128 + o_k1
        ...
        if K > 192:
            p_k = tl.make_block_ptr(k, (T_chunk, K), (H*K, 1), (i_t * CHUNK_SIZE, 192), (BT, 64), (1, 0))
            b_k = tl.load(p_k, boundary_check=(0, 1))
            if USE_GK:
                o_k4 = 192 + o_k1
        ...
        # Update dh
        p_w = tl.make_block_ptr(w, (K, T_chunk), (1, H*K), (0, i_t * CHUNK_SIZE), (64, BT), (0, 1))
        p_q = tl.make_block_ptr(q, (K, T_chunk), (1, H*K), (0, i_t * CHUNK_SIZE), (64, BT), (0, 1))
        b_w = tl.load(p_w, boundary_check=(0, 1))
        ...
        if K > 64:
            p_q = tl.make_block_ptr(q, (K, T_chunk), (1, H*K), (64, i_t * CHUNK_SIZE), (64, BT), (0, 1))
            p_w = tl.make_block_ptr(w, (K, T_chunk), (1, H*K), (64, i_t * CHUNK_SIZE), (64, BT), (0, 1))
            b_q = tl.load(p_q, boundary_check=(0, 1))
            b_w = tl.load(p_w, boundary_check=(0, 1))
            if USE_G:
                b_dh2 *= bg_last_exp
        ...
        if K > 128:
            p_q = tl.make_block_ptr(q, (K, T_chunk), (1, H*K), (128, i_t * CHUNK_SIZE), (64, BT), (0, 1))
            p_w = tl.make_block_ptr(w, (K, T_chunk), (1, H*K), (128, i_t * CHUNK_SIZE), (64, BT), (0, 1))
            b_q = tl.load(p_q, boundary_check=(0, 1))
            b_w = tl.load(p_w, boundary_check=(0, 1))
            if USE_G:
                b_dh3 *= bg_last_exp
        ...
        if K > 192:
            p_q = tl.make_block_ptr(q, (K, T_chunk), (1, H*K), (192, i_t * CHUNK_SIZE), (64, BT), (0, 1))
            p_w = tl.make_block_ptr(w, (K, T_chunk), (1, H*K), (192, i_t * CHUNK_SIZE), (64, BT), (0, 1))
            b_q = tl.load(p_q, boundary_check=(0, 1))
            b_w = tl.load(p_w, boundary_check=(0, 1))
            if USE_G:
                b_dh4 *= bg_last_exp
...
def chunk_gated_delta_rule_fwd_h(
    ...
    if cu_seqlens is None:
        ...
        N, NT, chunk_offsets = len(cu_seqlens) - 1, len(chunk_indices), prepare_chunk_offsets(cu_seqlens, CHUNK_SIZE)
    ...
    def grid(meta): return (N*H, triton.cdiv(V, meta['BV']))
