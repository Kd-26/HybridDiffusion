...
#
# Fixed wy_fast kernels (local copy for block_gated_delta_rule).
#
# FLA's original kernels place B*H in CUDA grid dim-y, which is limited to
# 65535. When the noisy batch is reshaped into (B*N, tbs, H, K) with large
# B*N, B*H exceeds 65535 and the kernel launch fails with "CUDA: invalid
# argument".
#
...
#
# Original: fla/ops/gated_delta_rule/wy_fast.py (fla 0.4.x, Songlin Yang & Yu Zhang)
...
def recompute_w_u_fwd_kernel(
    ...
    p_A = tl.make_block_ptr(A + (bos*H + i_h) * BT, (T_chunk, BT), (H*BT, 1), (i_t * CHUNK_SIZE, 0), (BT, BT), (1, 0))
    b_A = tl.load(p_A, boundary_check=(0, 1))

    for i_v in range(tl.cdiv(V, BV)):
        p_v = tl.make_block_ptr(v + (bos*H + i_h) * V, (T_chunk, V), (H*V, 1), (i_t * CHUNK_SIZE, i_v * BV), (BT, BV), (1, 0))
        p_u = tl.make_block_ptr(u + (bos*H + i_h) * V, (T_chunk, V), (H*V, 1), (i_t * CHUNK_SIZE, i_v * BV), (BT, BV), (1, 0))
        b_v = tl.load(p_v, boundary_check=(0, 1))
        b_vb = (b_v * b_b[:, None]).to(b_v.dtype)
        b_u = tl.dot(b_A, b_vb.to(b_A.dtype), allow_tf32=False)
    ...
    for i_k in range(tl.cdiv(K, BK)):
        ...
        p_w = tl.make_block_ptr(w + (bos*H + i_h) * K, (T_chunk, K), (H*K, 1), (i_t * CHUNK_SIZE, i_k * BK), (BT, BK), (1, 0))
        b_k = tl.load(p_k, boundary_check=(0, 1))
        b_kb = b_k * b_b[:, None]
        if USE_G:
            b_kb *= b_g[:, None]
        b_w = tl.dot(b_A, b_kb.to(b_A.dtype))
...
def prepare_wy_repr_bwd_kernel(
    ...
    for i_k in range(tl.cdiv(K, BK)):
        p_k = tl.make_block_ptr(k + (bos*H + i_h) * K, (T_chunk, K), (H*K, 1), (i_t * CHUNK_SIZE, i_k * BK), (BT, BK), (1, 0))
        p_dk = tl.make_block_ptr(dk + (bos*H + i_h) * K, (T_chunk, K), (H*K, 1), (i_t * CHUNK_SIZE, i_k * BK), (BT, BK), (1, 0))
        p_dw = tl.make_block_ptr(dw + (bos*H + i_h) * K, (T_chunk, K), (H*K, 1), (i_t * CHUNK_SIZE, i_k * BK), (BT, BK), (1, 0))
        b_k = tl.load(p_k, boundary_check=(0, 1))
        if USE_G:
        ...
        b_dkbg = tl.dot(b_A, b_dw.to(b_A.dtype))
        if USE_G:
    ...
    for i_v in range(tl.cdiv(V, BV)):
        ...
        p_du = tl.make_block_ptr(du + (bos*H + i_h) * V, (T_chunk, V), (H*V, 1), (i_t * CHUNK_SIZE, i_v * BV), (BT, BV), (1, 0))
        b_v = tl.load(p_v, boundary_check=(0, 1))
        b_vb = (b_v * b_b[:, None]).to(b_v.dtype)
        b_du = tl.load(p_du, boundary_check=(0, 1))
        b_dA += tl.dot(b_du, tl.trans(b_vb).to(b_du.dtype))
        b_dvb = tl.dot(b_A, b_du.to(b_A.dtype))
        b_dv = b_dvb * b_b[:, None]
    ...
    tl.store(p_db, b_db.to(db.dtype.element_ty), mask=m_t)
...
def prepare_wy_repr_bwd(
    ...
    # Fixed: grid (B*H, NT) instead of (NT, B*H)
    prepare_wy_repr_bwd_kernel[(B * H, NT)](
        k=k,
        v=v,
        beta=beta,
        g=g,
