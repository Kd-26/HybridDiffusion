...
class Qwen3Model(nn.Module, ModelProtocol):
    ...
    def _get_flex_attention_masks(
        ...
        from torchtitan.models.attention import get_doc_id_mask_mod
    ...
    def get_attention_masks(
        ...
        match self.model_args.attn_type:
            ...
            case "varlen":
                if self.model_args.attn_mask_type == "doc_causal" and \
                        extra_inputs and "doc_ids" in extra_inputs:
                    from torchtitan.models.attention import create_varlen_metadata_from_doc_ids
    ...
    def forward(
        ...
        **kwargs,
        ...
        Args:
            ...
            attention_masks (AttentionMasksType | None): Masks used when calculating attention scores.
            positions (torch.Tensor | None): Position indices for RoPE.
            labels (torch.Tensor | None): Target labels for fused CE. When provided
                and liger fused CE is available, loss is computed internally to avoid
                materializing the full logits tensor. Returns ``{"loss": loss}`` dict.
                When None, returns logits tensor as before.
            **kwargs: Extra inputs from the dataset (e.g. ``doc_ids``). These are
                consumed by ``get_attention_masks()`` before the forward call and
                are ignored here.

        Returns:
            torch.Tensor or dict: Logits (B, L, V) or {"loss": scalar} if fused CE used.
        """
        # pyrefly: ignore[not-callable, invalid-argument]
        h = self.tok_embeddings(tokens) if self.tok_embeddings else tokens

        if positions is None and "doc_ids" in kwargs:
            from torchtitan.models.qwen3.model.model_dllm import _doc_ids_to_positions
            positions = _doc_ids_to_positions(kwargs["doc_ids"])
