...
class VarlenAttentionWrapper(torch.nn.Module):
    """Variable-length attention using per-sequence causal boundaries.

    On torch >= 2.10, uses ``torch.nn.attention.varlen.varlen_attn``.
    On torch < 2.10 (e.g. 2.9), falls back to ``flash_attn.flash_attn_varlen_func``.

    Input tensors are expected in standard 4D format ``[B, n_heads, S, head_dim]``
    (same as other attention wrappers). Output is also 4D so the caller's
    ``transpose(1, 2).view(bs, seqlen, -1)`` chain works correctly.
    """

    _compiled_varlen_attn: ClassVar[Callable] = None
    _use_flash_attn: ClassVar[bool] = False

    def __init__(self):
        super().__init__()
        if VarlenAttentionWrapper._compiled_varlen_attn is not None:
            return
        # Prefer flash_attn_varlen_func (correct numerics) over
        # torch.nn.attention.varlen.varlen_attn (output range bug).
        try:
            from flash_attn import flash_attn_varlen_func
            VarlenAttentionWrapper._use_flash_attn = True
            VarlenAttentionWrapper._compiled_varlen_attn = flash_attn_varlen_func
        except ImportError:
            if varlen_attn is not None:
                VarlenAttentionWrapper._compiled_varlen_attn = torch.compile(
                    varlen_attn, mode="max-autotune-no-cudagraphs"
                )
            else:
                raise ImportError(
                    "VarlenAttentionWrapper requires either flash-attn or "
                    "torch >= 2.10. Install with: "
    ...
    def forward(
        ...
        # Reshape from [B, n_heads, S, head_dim] to [B*S, n_heads, head_dim]
        ...
        if VarlenAttentionWrapper._use_flash_attn:
            # flash_attn_varlen_func expects (total, n_heads, head_dim)
            # and returns (total, n_heads, head_dim)
            output = VarlenAttentionWrapper._compiled_varlen_attn(
                xq_packed,
                xk_packed,
                xv_packed,
                cu_seq_q,
        ...
        # Reshape back to 4D: [B*S, n_heads, head_dim] -> [B, n_heads, S, head_dim]
        # The caller does output.transpose(1, 2).view(bs, seqlen, -1)
        output = output.view(bs, seqlen, n_local_heads, head_dim).transpose(1, 2)
        return output
...
class FlexAttentionWrapper(torch.nn.Module):
    ...
    _compiled_flex_attn: ClassVar[Callable] = None
    ...
    def __init__(self):
        ...
        if FlexAttentionWrapper._compiled_flex_attn is None:
            ...
            # wrap_inductor_compiled_regions is only available in torch >= 2.10
            if _TORCH_VERSION >= (2, 10):
                _compile_options["wrap_inductor_compiled_regions"] = True
            FlexAttentionWrapper._compiled_flex_attn = torch.compile(
                flex_attention, options=_compile_options,
            )
...
def get_doc_id_mask_mod(doc_ids: torch.Tensor) -> _mask_mod_signature:
    """Creates a document mask from explicit doc_ids tensor.

    Args:
        doc_ids: Integer tensor of shape [B, L] where each value is a
            document ID. Tokens with the same doc_id belong to the same
            document and can attend to each other.

    Returns:
        A mask modifier that returns True when query and key positions
...
def create_varlen_metadata_from_doc_ids(
    doc_ids: torch.Tensor,
) -> VarlenMetadata:
    """Create VarlenMetadata from explicit doc_ids tensor.

    Args:
        doc_ids: Integer tensor of shape [B, L] where each value is a
            document ID. Consecutive positions with the same ID belong
            to the same document.

    Returns:
        VarlenMetadata with cumulative sequence lengths for varlen attention.
    """
    ...
    for b in range(batch_size):
        ids = doc_ids[b]
        # Find positions where doc_id changes
        changes = (ids[1:] != ids[:-1]).nonzero(as_tuple=True)[0] + 1
        boundaries = torch.cat([
            torch.tensor([0], dtype=torch.int32, device=device),
            changes.to(torch.int32),
            torch.tensor([seq_len], dtype=torch.int32, device=device),
        ])
        seq_lengths = torch.diff(boundaries)
        batch_max = seq_lengths.max().item()
        if batch_max > max_seqlen:
            max_seqlen = batch_max
        cu_seqlens_list.append(boundaries[:-1] + offset)
        offset += seq_len
