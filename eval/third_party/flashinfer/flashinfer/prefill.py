...
def get_batch_prefill_module(backend, *args):
    ...
    def ragged_run(
        ...
        if backend == "fa2":
            ragged_run_func(
                ...
                None,  # maybe_dllm_mask_type, paged native dLLM only
                logits_soft_cap,
                sm_scale,
                1.0 / rope_scale,  # rope_rcp_scale
                1.0 / rope_theta,  # rope_rcp_theta,
                token_pos_in_items_len,
                0,  # dllm_gen_block_size
                0,  # dllm_block_size
            )
        elif is_fp8:
    ...
    def paged_run(
        ...
        maybe_token_pos_in_items_ptr: Optional[torch.Tensor],
        maybe_max_item_len_ptr: Optional[torch.Tensor],
        maybe_dllm_mask_type: Optional[torch.Tensor],
        logits_soft_cap: float,
        sm_scale: float,
        scale_q: Optional[torch.Tensor],
        scale_k: Optional[torch.Tensor],
...
class BatchPrefillWithPagedKVCacheWrapper:
    ...
    def __init__(
        ...
        self._dllm_bidir_block_mask = False
        self._dllm_mask_type = None
        self._dllm_gen_block_size = 0
        self._dllm_block_size = 0
    ...
    def plan(
        ...
        packed_custom_mask: Optional[torch.Tensor] = None,
        causal: bool = False,
        dllm_bidir_block_mask: bool = False,
        dllm_mask_type: Optional[torch.Tensor] = None,
        dllm_gen_block_size: Optional[int] = None,
        ...
        causal : bool
            ...
            :meth:`plan`.
        dllm_bidir_block_mask : bool
            Whether to use the native dLLM V3/V4 structured block mask. This
            mode avoids materializing a custom mask and is only supported by
            the FA2 paged prefill backend.
        dllm_mask_type : Optional[torch.Tensor]
            Optional int32 per-request mask type tensor. ``0`` selects normal
            causal append masking and ``1`` selects the structured dLLM block
            mask. If omitted with ``dllm_bidir_block_mask=True``, all requests
            use the structured dLLM block mask.
        dllm_gen_block_size : Optional[int]
            Number of generated draft/MASK positions in the dLLM block.
        dllm_block_size : Optional[int]
            Total dLLM block size.
        pos_encoding_mode : str
            The position encoding applied inside attention kernels, could be
            ``NONE``/``ROPE_LLAMA`` (LLAMA style rotary embedding) /``ALIBI``.
            Default is ``NONE``.
        ...
        if fixed_split_size is None:
            fixed_split_size = -1
        if dllm_bidir_block_mask:
            if custom_mask is not None or packed_custom_mask is not None:
                raise ValueError(
                    "dllm_bidir_block_mask cannot be combined with custom_mask or packed_custom_mask."
                )
            if prefix_len_ptr is not None:
                raise ValueError(
                    "dllm_bidir_block_mask cannot be combined with multi-item scoring mask."
                )
            if dllm_mask_type is not None and dllm_mask_type.dtype != torch.int32:
                raise ValueError("dllm_mask_type must be an int32 tensor.")
            if dllm_mask_type is not None and len(dllm_mask_type) != len(qo_indptr) - 1:
                raise ValueError("dllm_mask_type must have one entry per request.")
        ...
        self._dllm_mask_type = dllm_mask_type
        self._dllm_gen_block_size = (
            int(dllm_gen_block_size) if dllm_gen_block_size is not None else 0
        )
        self._dllm_block_size = int(dllm_block_size) if dllm_block_size is not None else 0
        ...
        if self._jit_module is not None:
            ...
            if dllm_bidir_block_mask and self._backend != "fa2":
                raise NotImplementedError(
                    "dllm_bidir_block_mask is currently supported only by the FA2 paged prefill backend."
                )
        ...
        self._causal = causal
        self._dllm_bidir_block_mask = dllm_bidir_block_mask
    ...
    def run(
        ...
        *args
        ...
        if self._dllm_bidir_block_mask:
            mask_mode = (
                MaskMode.DLLM_TYPED_BIDIR_BLOCK.value
                if self._dllm_mask_type is not None
                else MaskMode.DLLM_BIDIR_BLOCK.value
            )
