...
def _get_cubin_dir():
    ...
    if has_flashinfer_cubin():
        ...
        if (
            ...
            and _public_version(flashinfer_version)
            != _public_version(flashinfer_cubin_version)
        ):
            raise RuntimeError(
...
def _get_aot_dir():
    ...
    if has_flashinfer_jit_cache():
        ...
        ) and not _public_version(flashinfer_jit_cache_version).startswith(
            _public_version(flashinfer_version)
        ):
            raise RuntimeError(
...
def _source_or_data_path(source_path: pathlib.Path, data_path: pathlib.Path) -> pathlib.Path:
    return source_path if source_path.exists() else data_path


FLASHINFER_INCLUDE_DIR: pathlib.Path = _source_or_data_path(
    _source_root / "include", _package_root / "data" / "include"
)
FLASHINFER_CSRC_DIR: pathlib.Path = _source_or_data_path(
    _source_root / "csrc", _package_root / "data" / "csrc"
)
# FLASHINFER_SRC_DIR = _package_root / "data" / "src"
CUTLASS_INCLUDE_DIRS: list[pathlib.Path] = [
    _source_or_data_path(
        _source_root / "3rdparty" / "cutlass" / "include",
        _package_root / "data" / "cutlass" / "include",
    ),
    _source_or_data_path(
        _source_root / "3rdparty" / "cutlass" / "tools" / "util" / "include",
        _package_root / "data" / "cutlass" / "tools" / "util" / "include",
    ),
]
SPDLOG_INCLUDE_DIR: pathlib.Path = _source_or_data_path(
    _source_root / "3rdparty" / "spdlog" / "include",
    _package_root / "data" / "spdlog" / "include",
)
