...
#pragma unroll
      for (uint32_t reg_id = 0; reg_id < 8; ++reg_id) {
        const uint32_t q_idx = q[mma_q][(reg_id % 4) / 2], kv_idx = kv_idx_base + mma_kv * 16 +
                                                                    ...
                                                                    8 * (reg_id / 4) + reg_id % 2;
        const uint32_t qo_head_idx = kv_head_idx * group_size + r[mma_q][(reg_id % 4) / 2];
        bool mask = !(MASK_MODE == MaskMode::kCausal || MASK_MODE == MaskMode::kMultiItemScoring
                          ? (kv_idx + qo_len > kv_len + q_idx || (kv_idx >= chunk_end))
                          : kv_idx >= chunk_end);
        if constexpr (MASK_MODE == MaskMode::kDllmBidirBlock) {
          const uint32_t prefix_len = kv_len - qo_len;
          const uint32_t gen_block_size =
              params.dllm_gen_block_size != 0 ? params.dllm_gen_block_size : (qo_len + 1) / 2;
          const uint32_t block_size =
              params.dllm_block_size != 0 ? params.dllm_block_size : qo_len;
          const bool in_prefix = kv_idx < prefix_len;
          const bool in_local_block = kv_idx >= prefix_len && kv_idx < prefix_len + block_size;
          const uint32_t local_col = kv_idx - prefix_len;
          const bool local_visible =
              in_local_block && (q_idx >= gen_block_size || local_col <= q_idx);
          mask = (q_idx < qo_len) && (kv_idx < chunk_end) && (in_prefix || local_visible);
        }
        if constexpr (MASK_MODE == MaskMode::kDllmTypedBidirBlock) {
          const bool use_causal =
              params.maybe_dllm_mask_type != nullptr &&
              __ldg(params.maybe_dllm_mask_type + batch_idx) == 0;
          if (use_causal) {
            mask = (kv_idx + qo_len <= kv_len + q_idx) && (kv_idx < chunk_end);
          } else {
            const uint32_t prefix_len = kv_len - qo_len;
            const uint32_t gen_block_size =
                params.dllm_gen_block_size != 0 ? params.dllm_gen_block_size : (qo_len + 1) / 2;
        ...
        mask = mask && variant.LogitsMask(params, batch_idx, q_idx, kv_idx, qo_head_idx,
                                          kv_head_idx);
        s_frag[mma_q][mma_kv][reg_id] =
...
__device__ __forceinline__ void SinglePrefillWithKVCacheDevice(
  ...
  if constexpr (std::is_same_v<DTypeQ, nv_bfloat16>) {
    ...
    const uint32_t num_iterations = ceil_div(
        (MASK_MODE == MaskMode::kCausal || MASK_MODE == MaskMode::kDllmBidirBlock ||
         MASK_MODE == MaskMode::kDllmTypedBidirBlock)
            ? min(chunk_size,
...
#pragma unroll 1
    for (uint32_t iter = 0; iter < num_iterations; ++iter) {
      ...
      // DLLM block masks differ from causal masking only in the local suffix
      // block. Prefix-only KV tiles are fully visible and can use the causal
      // fast path that skips per-score mask predicates.
      const uint32_t prefix_len = sub_if_greater_or_zero(kv_len, qo_len);
      const uint32_t kv_tile_end = kv_idx_base + NUM_MMA_KV * 16;
      const bool dllm_prefix_only_tile =
          (MASK_MODE == MaskMode::kDllmBidirBlock ||
           MASK_MODE == MaskMode::kDllmTypedBidirBlock) &&
          kv_tile_end <= prefix_len && kv_tile_end <= chunk_end;

      // apply mask
      if (MASK_MODE == MaskMode::kCustom ||
          ((MASK_MODE == MaskMode::kDllmBidirBlock ||
            MASK_MODE == MaskMode::kDllmTypedBidirBlock)
               ? !dllm_prefix_only_tile
