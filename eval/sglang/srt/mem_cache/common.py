...
def release_kv_cache(req: Req, tree_cache: BasePrefixCache, is_insert: bool = True):
    ...
    if req.req_pool_idx is None:
        ...
        if isinstance(tree_cache.req_to_token_pool, HybridReqToTokenPool):
            tree_cache.req_to_token_pool.free_mamba_cache(req)
        elif req.mamba_pool_idx is not None:
    ...
    if req.is_dllm():
        boundary_insert = getattr(tree_cache, "cache_dllm_generated_boundary", None)
        if boundary_insert is not None:
            boundary_insert(
                req,
                getattr(req, "mamba_last_track_seqlen", None),
                reason="release_kv_cache",
            )
        is_insert = False
        mamba_owner_trace(
            "release_kv_cache_dllm_skip_insert",
            getattr(req, "mamba_pool_idx", None),
            req_pool_idx=req.req_pool_idx,
            rid=getattr(req, "rid", None),
            has_ping_pong=getattr(req, "mamba_ping_pong_track_buffer", None) is not None,
    ...
    # dLLM requests intentionally skip insertion into radix cache, so their
    # mamba state is not owned by the cache even when the cache supports mamba.
    if isinstance(tree_cache.req_to_token_pool, HybridReqToTokenPool) and (
        not tree_cache.supports_mamba() or req.is_dllm()
    ):
        if (
