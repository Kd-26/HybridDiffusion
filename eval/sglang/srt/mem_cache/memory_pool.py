...
def mamba_owner_trace(event: str, slots=None, **kwargs):
    if os.getenv("SGLANG_HYBRID_DIFFUSION_SELF_SPEC_MAMBA_OWNER_TRACE", "0") != "1":
        return
    if isinstance(slots, torch.Tensor):
        slots_repr = slots.detach().cpu().view(-1).tolist()
    elif slots is None:
        slots_repr = None
    elif isinstance(slots, (list, tuple)):
        slots_repr = [
            x.detach().cpu().item() if isinstance(x, torch.Tensor) else x
            for x in slots
        ]
    else:
        slots_repr = slots
    logger.warning("[MAMBA_OWNER] event=%s slots=%s meta=%s", event, slots_repr, kwargs)
...
class MambaPool:
    ...
    def alloc(self, need_size: int) -> Optional[torch.Tensor]:
        ...
        mamba_owner_trace(
            ...
            available_after=len(self.free_slots),
        )
    ...
    def free(self, free_index: torch.Tensor):
        ...
        mamba_owner_trace(
            "pool_free",
            free_index,
            available_before=len(self.free_slots),
        )
    ...
    def fork_from(self, src_index: torch.Tensor) -> Optional[torch.Tensor]:
        ...
        mamba_owner_trace("pool_fork", dst_index, src=src_index)
        return dst_index
...
class HybridReqToTokenPool(ReqToTokenPool):
    ...
    def free_mamba_cache(
        ...
        req_pool_idx = req.req_pool_idx
        if mamba_index is None and req_pool_idx is not None:
            mapped = self.req_index_to_mamba_index_mapping[req_pool_idx]
            if mapped.item() > 0:
                mamba_index = mapped.to(dtype=torch.int64)
        if mamba_index is not None:
            mamba_owner_trace(
                "hybrid_free_req_slot",
                mamba_index,
                req_pool_idx=req_pool_idx,
                rid=getattr(req, "rid", None),
                keep=mamba_ping_pong_track_buffer_to_keep,
            )
            self.mamba_pool.free(mamba_index.unsqueeze(0))
            req.mamba_pool_idx = None
            if req_pool_idx is not None:
                self.req_index_to_mamba_index_mapping[req_pool_idx] = 0

        if self.enable_mamba_extra_buffer:
            mamba_ping_pong_track_buffer_to_free = req.mamba_ping_pong_track_buffer
            if mamba_ping_pong_track_buffer_to_free is None and req_pool_idx is not None:
                mapped = self.req_index_to_mamba_ping_pong_track_buffer_mapping[
                    req_pool_idx
                ]
                if torch.any(mapped > 0):
                    mamba_ping_pong_track_buffer_to_free = mapped.to(dtype=torch.int64)
            if mamba_ping_pong_track_buffer_to_free is None:
                return
            ...
            if mamba_ping_pong_track_buffer_to_keep is None:
                ...
                req.mamba_ping_pong_track_buffer_cpu = None
                if req_pool_idx is not None:
