...
class ForwardMode(IntEnum):
    ...
    # Used in dLLM
    DLLM_EXTEND = auto()
    # dLLM mixed inline prefill + decode. This has extend semantics but is not
    # CUDA graphable because it contains real prefill work.
    DLLM_MIXED = auto()
    ...
    def is_extend(self, include_draft_extend_v2: bool = False):
        return (
            ...
            or self == ForwardMode.DLLM_MIXED
        )
    ...
    def is_dllm_extend(self):
        return self == ForwardMode.DLLM_EXTEND or self == ForwardMode.DLLM_MIXED

    def is_dllm_mixed(self):
        return self == ForwardMode.DLLM_MIXED

    def is_dllm_mode(self):
        return self == ForwardMode.DLLM_EXTEND or self == ForwardMode.DLLM_MIXED
...
class ForwardBatch(ForwardBatchDeepSeekMHAMixin):
    ...
    # Requested paged-mask contract: auto, native, custom, or full.  Runtime
    # selection is recorded separately so instrumentation cannot confuse intent
    # with the backend that actually executed.
    ...
    def init_new(
        ...
        # DLLM prefill can carry variable token counts, so it must use the
        # standard extend-position path. Mixed inline prefill needs per-request
        # positions: prefill entries use prefix positions, decode entries use
        # the same block-offset positions as pure DLLM_EXTEND.
        dllm_mask_types_cpu = getattr(batch, "dllm_attn_mask_types_cpu", None)
        ...
        if region_dag_specs is not None:
            ...
            and batch.extend_prefix_lens is not None
        ):
            from sglang.srt.dllm.config import DLLM_ATTN_MASK_CAUSAL_PREFILL

            positions_dtype = torch.int64 if is_hip() or _is_npu else torch.int32
            ret.positions = torch.tensor(
                [
                    pos
                    for mask_type, prefix_len, extend_len, block_offset in zip(
                        dllm_mask_types_cpu,
        ...
        elif batch.dllm_config is not None and not dllm_is_prefill:
        ...
        if getattr(batch, "dllm_attn_mask_types_cpu", None) is not None:
            ret.dllm_attn_mask_types_cpu = batch.dllm_attn_mask_types_cpu
            ret.dllm_attn_mask_types = torch.tensor(
                batch.dllm_attn_mask_types_cpu,
                dtype=torch.int32,
                device=device,
            )
