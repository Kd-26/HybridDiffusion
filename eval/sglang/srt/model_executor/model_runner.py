...
_AR_SAMPLE_TIMING_ENABLED = os.getenv("SGLANG_AR_SAMPLE_TIMING", "0") == "1"
_AR_SAMPLE_TIMING_SYNC = os.getenv("SGLANG_AR_SAMPLE_TIMING_SYNC", "1") == "1"
_AR_SAMPLE_TIMING_INTERVAL = int(os.getenv("SGLANG_AR_SAMPLE_TIMING_INTERVAL", "100"))
...
class ModelRunner(ModelRunnerKVCacheMixin):
    ...
    def sample(
        ...
        sample_timing = (
            _AR_SAMPLE_TIMING_ENABLED
            and forward_batch.forward_mode.is_decode()
            and not forward_batch.forward_mode.is_dllm_extend()
        )
        if sample_timing and _AR_SAMPLE_TIMING_SYNC and torch.cuda.is_available():
            torch.cuda.synchronize()
        if sample_timing:
        ...
        if sample_timing and _AR_SAMPLE_TIMING_SYNC and torch.cuda.is_available():
            torch.cuda.synchronize()
        if sample_timing:
            _t_sampler_start = time.perf_counter()
        ...
        if sample_timing and _AR_SAMPLE_TIMING_SYNC and torch.cuda.is_available():
            torch.cuda.synchronize()
        if sample_timing:
            _t_sampler_end = time.perf_counter()
            stats = getattr(self, "_ar_sample_timing", None)
            if stats is None:
                stats = {
                    "count": 0,
                    "preprocess": 0.0,
                ...
                self._ar_sample_timing = stats
            stats["count"] += 1
            stats["preprocess"] += _t_sampler_start - _t_preprocess_start
            stats["sampler"] += _t_sampler_end - _t_sampler_start
            stats["batch_size"] += forward_batch.batch_size
            if stats["count"] % _AR_SAMPLE_TIMING_INTERVAL == 0:
                count = max(stats["count"], 1)
                logger.info(
                    "[AR_SAMPLE_TIMING] steps=%s avg_bs=%.2f preprocess=%.3fms sampler=%.3fms total=%.3fms sync=%s",
                    stats["count"],
                    stats["batch_size"] / count,
                    stats["preprocess"] / count * 1000.0,
                    stats["sampler"] / count * 1000.0,
                    (stats["preprocess"] + stats["sampler"]) / count * 1000.0,
                    _AR_SAMPLE_TIMING_SYNC,
                )
