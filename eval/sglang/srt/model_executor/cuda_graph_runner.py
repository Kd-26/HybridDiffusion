...
from sglang.srt.dllm.config import DllmConfig, SelfSpecVariant
    ...
    def populate_from_forward_batch(
        ...
        *,
        ...
        if forward_batch.extend_prefix_lens is not None:
            dsts.append(self.extend_prefix_lens[:raw_bs])
            srcs.append(forward_batch.extend_prefix_lens)
        if forward_batch.extend_seq_lens is not None:
            dsts.append(self.extend_seq_lens[:raw_bs])
            srcs.append(forward_batch.extend_seq_lens)
...
class CudaGraphRunner:
    ...
    def __init__(
        ...
        *,
        ...
        archs = getattr(model_runner.model_config.hf_config, "architectures", []) or []
        model_type = getattr(model_runner.model_config.hf_config, "model_type", None)
        self.is_gdn_dllm = self.is_dllm and (
            model_type == "qwen3_5"
            or any("Qwen3_5DLLM" in arch for arch in archs)
        )
    ...
    def can_run(self, forward_batch: ForwardBatch):
        ...
        if self.is_dllm:
            dllm_shape_supported = (
                forward_batch.batch_size * self.num_tokens_per_bs
                == forward_batch.input_ids.numel()
            )
            dllm_mask_types_cpu = getattr(
                forward_batch, "dllm_attn_mask_types_cpu", None
            )
            expected_gdn_causal_mode = (
                2
                if self.dllm_config.variant
                in (
                    SelfSpecVariant.BD_BIDIR,
                    SelfSpecVariant.BD_BIDIR_SHIFT,
                    SelfSpecVariant.HYBRID_DIFFUSION_SHIFT,
                )
                else 1
            )
            dllm_custom_mask = getattr(forward_batch, "dllm_bidir_custom_mask", None)
            dllm_custom_mask_supported = (
                dllm_custom_mask is None
                or (
                    expected_gdn_causal_mode == 2
                    and dllm_custom_mask.ndim == 2
                    and dllm_custom_mask.shape[0] == self.num_tokens_per_bs
                    and dllm_custom_mask.shape[1] == self.num_tokens_per_bs
                )
            )
            is_hybrid_gdn_model = self.model_runner.hybrid_gdn_config is not None
            dllm_gdn_semantics_supported = (
                not is_hybrid_gdn_model
                or (
                    bool(
                        getattr(
                            forward_batch,
                            "dllm_gdn_cache_intermediate_for_commit",
                            False,
                        )
                    )
                    and not bool(
                        getattr(forward_batch, "dllm_gdn_save_for_commit", False)
                    )
                    and not bool(
                        getattr(forward_batch, "dllm_gdn_persist_state", True)
                    )
                    and int(getattr(forward_batch, "dllm_gdn_block_size", -1))
                    == self.num_tokens_per_bs
                    and int(getattr(forward_batch, "dllm_gdn_causal_mode", -1))
                    == expected_gdn_causal_mode
                )
            )
            dllm_semantics_supported = (
                dllm_custom_mask_supported and dllm_gdn_semantics_supported
            )
            is_dllm_supported = (
                forward_batch.forward_mode == ForwardMode.DLLM_EXTEND
                and dllm_shape_supported
                and dllm_mask_types_cpu is None
                and dllm_semantics_supported
            )
            if os.getenv("SGLANG_DLLM_LOG_GRAPH", "0") == "1":
                log_count = getattr(self, "_dllm_graph_log_count", 0)
                if log_count < 20 or log_count % 500 == 0:
                    logger.info(
                        "[DLLM cuda graph] can_run=%s bs=%s tokens=%s expected=%s "
                        "mode=%s mask_types=%s semantics=%s hybrid_gdn=%s "
                        "gdn_semantics=%s "
                        "cache_intermediate=%s save_commit=%s persist=%s "
                        "block_size=%s gdn_mode=%s custom_mask=%s",
                        bool(is_dllm_supported),
                        forward_batch.batch_size,
                        forward_batch.input_ids.numel(),
                        forward_batch.batch_size * self.num_tokens_per_bs,
                        forward_batch.forward_mode,
                        "none" if dllm_mask_types_cpu is None else "present",
                        bool(dllm_semantics_supported),
                        bool(is_hybrid_gdn_model),
                        bool(dllm_gdn_semantics_supported),
                        bool(
                            getattr(
                                forward_batch,
                                "dllm_gdn_cache_intermediate_for_commit",
                        ...
                        bool(getattr(forward_batch, "dllm_gdn_save_for_commit", False)),
                        bool(getattr(forward_batch, "dllm_gdn_persist_state", True)),
                        int(getattr(forward_batch, "dllm_gdn_block_size", -1)),
                        int(getattr(forward_batch, "dllm_gdn_causal_mode", -1)),
                        (
                            "none"
                            if dllm_custom_mask is None
                            else tuple(dllm_custom_mask.shape)
                        ),
                    )
                self._dllm_graph_log_count = log_count + 1
        else:
            is_dllm_supported = True
    ...
    def capture_one_batch_size(
        ...
        if self.is_dllm:
            extend_prefix_lens = buffers.extend_prefix_lens[:bs]
            extend_seq_lens = buffers.extend_seq_lens[:bs]
            extend_prefix_lens.copy_(
                torch.clamp(seq_lens.to(torch.int32) - self.num_tokens_per_bs, min=0)
            )
            extend_seq_lens.fill_(self.num_tokens_per_bs)
            extend_prefix_lens_cpu = [0] * bs
            extend_seq_lens_cpu = [self.num_tokens_per_bs] * bs
        if self.is_encoder_decoder:
        ...
        forward_batch = ForwardBatch(
            ...
            positions=positions,
            extend_num_tokens=num_tokens if self.is_dllm else None,
            extend_seq_lens=extend_seq_lens,
            extend_prefix_lens=extend_prefix_lens,
        ...
        if self.is_dllm:
            if self.dllm_config.variant in (
                SelfSpecVariant.BD_BIDIR,
                SelfSpecVariant.BD_BIDIR_SHIFT,
                SelfSpecVariant.HYBRID_DIFFUSION_SHIFT,
            ):
                ...
                forward_batch.dllm_gdn_num_clean = (
                    self.dllm_config.algorithm_config.get("gen_block_size", 3)
                )
            else:
            ...
            forward_batch.dllm_gdn_block_size = self.num_tokens_per_bs
            dllm_algorithm_config = self.dllm_config.algorithm_config
            dllm_temperature = float(dllm_algorithm_config.get("temperature", 1.0))
            dllm_top_k = int(dllm_algorithm_config.get("top_k", 50))
            dllm_requires_full_logits = (
                self.dllm_config.algorithm == "LowConfidenceShiftHybridDiffusion"
            )
            dllm_fast_verify = bool(dllm_algorithm_config.get("fast_verify", False))
            dllm_draft_mode = str(
                dllm_algorithm_config.get(
                    "draft_mode",
                    dllm_algorithm_config.get("draft_model", "strict_truncated"),
                )
            )
            forward_batch.dllm_return_argmax_only = (
                dllm_temperature <= 0 and not dllm_requires_full_logits
            )
            forward_batch.dllm_return_topk_probs = (
                dllm_temperature > 0
                and dllm_top_k > 0
                and not dllm_fast_verify
                and dllm_draft_mode != "argmax_softmax_verify"
                and not dllm_requires_full_logits
            )
            forward_batch.dllm_top_k = dllm_top_k
            forward_batch.dllm_top_p = float(dllm_algorithm_config.get("top_p", 1.0))
            forward_batch.dllm_temperature = dllm_temperature
    ...
    def replay(
        ...
        if isinstance(output, LogitsProcessorOutput):
            if self.is_dllm:
                ...
                full_argmax = (
                    output.full_argmax[: self.raw_num_token]
                    if output.full_argmax is not None
                    else None
                )
                full_topk_ids = (
                    output.full_topk_ids[: self.raw_num_token]
                    if output.full_topk_ids is not None
                    else None
                )
                full_topk_probs = (
                    output.full_topk_probs[: self.raw_num_token]
                    if output.full_topk_probs is not None
                    else None
                )
            else:
                full_logits = None
            ...
            return LogitsProcessorOutput(
                ...
                full_topk_ids=full_topk_ids,
                full_topk_probs=full_topk_probs,
                hidden_states=(
