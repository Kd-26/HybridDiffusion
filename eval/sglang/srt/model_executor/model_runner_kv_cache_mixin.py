...
class ModelRunnerKVCacheMixin:
    def _get_mamba_spec_num_draft_tokens(self: ModelRunner) -> int | None:
        """Number of per-step intermediate states needed by hybrid Mamba/GDN paths."""
        if self.server_args.speculative_num_draft_tokens is not None:
            return self.server_args.speculative_num_draft_tokens
        if self.server_args.dllm_algorithm is not None:
            from sglang.srt.dllm.config import DllmConfig

            return DllmConfig.from_server_args(self.server_args).block_size
        return None
    ...
    def handle_max_mamba_cache(self: ModelRunner, total_rest_memory):
        ...
        # Reserve memory for intermediate Mamba/GDN states used by speculative
        # verify paths and by dLLM accepted-state scatter commit.
        spec_num_draft_tokens = self._get_mamba_spec_num_draft_tokens()
        if not self.spec_algorithm.is_none() or server_args.dllm_algorithm is not None:
            assert spec_num_draft_tokens is not None
    ...
    def _init_pools(self: ModelRunner):
        ...
        if self.req_to_token_pool is None:
            ...
            if spec_num_draft_tokens is not None:
                extra_max_context_len += spec_num_draft_tokens
