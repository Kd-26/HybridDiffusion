...
class ModelConfig:
    def __init__(
        ...
        self.is_dllm_model = is_dllm_model
        ...
        if self.is_dllm_model:
            dllm_arch_overrides = {
                "Qwen3_5ForCausalLM": "Qwen3_5DLLMForCausalLM",
                "Qwen3_5ForConditionalGeneration": "Qwen3_5DLLMForConditionalGeneration",
            }
            arch = self.hf_config.architectures[0]
            if arch in dllm_arch_overrides:
                self.hf_config.architectures[0] = dllm_arch_overrides[arch]
                logger.info(
                    "Using %s for diffusion LLM inference",
        ...
        if enable_multimodal is None:
            mm_disabled_models = [
                ...
                # Qwen3.5 DLLM checkpoints are served as pure language models.
                # The copied HF config still has a vision_config because it
                # reuses Qwen3.5's conditional-generation wrapper, but text-only
                # Text-only self-spec inference must not initialize the vision preprocessor.
    ...
    def from_server_args(
        ...
        **kwargs,
        ...
        return ModelConfig(
            ...
            is_dllm_model=server_args.dllm_algorithm is not None,
