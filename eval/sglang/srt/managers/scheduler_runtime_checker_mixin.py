...
from sglang.srt.mem_cache.memory_pool import mamba_owner_trace
...
class SchedulerRuntimeCheckerMixin:
    ...
    def _check_mamba_pool(self: Scheduler, ps: PoolStats) -> Tuple[bool, str]:
        ...
        if leak:
            ...
            # MambaPool reserves padded slot 0 for dummy writes and allocates
            # real request/cache slots from 1..size inclusive.
            ...
            mamba_owner_trace(
                "scheduler_mamba_leak",
                sorted(leaked_mamba_pages),
                free=sorted(free_mamba_pages)[:32],
                cached=sorted(cached_mamba_pages)[:32],
                leaked_full_pages=sorted(leaked_full_pages)[:32],
            )
            msg += (
