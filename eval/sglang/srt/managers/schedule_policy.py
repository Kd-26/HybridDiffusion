...
class PrefillAdder:
    ...
    def _init_dllm_meta(self, dllm_config: DllmConfig):
        ...
        # Whole-prompt dLLM prefill is allowed; only decode is capped by
        # block_size. Keep a separate DLLM budget, but make sure prefill is not
        # artificially truncated to one block.
    ...
    def _add_dllm_req(
        ...
        if req.is_dllm_prefill():
            ...
            if trunc_len < origin_remaining:
                trunc_len = trunc_len // self.page_size * self.page_size
        else:
            # Decode stays block-sized. For Qwen3.5 extra_buffer the radix page
            # is FLA_CHUNK_SIZE (64), while a self-spec round is much smaller
            # (typically block_size=7). Page-aligning decode would round to 0
            # and prevent staged decode requests from joining the running batch.
            trunc_len = min(
                self.rem_dllm_tokens,
                self.dllm_block_size,
                int(self.rem_total_tokens),
    ...
    def add_one_req_ignore_eos(self, req: Req):
        ...
        if self.dllm_config is not None:
            ...
            if not self._add_dllm_req(req, len(req.prefix_indices)):
                return AddReqResult.OTHER
    ...
    def add_one_req(
        ...
        with self._lock_node(req.last_node):
            ...
            if self.dllm_config is not None:
                ...
                if not self._add_dllm_req(req, prefix_len, truncation_align_size):
                    return AddReqResult.OTHER
                self._req_inc_lock_ref(req)
