...
_AR_TIMING_ENABLED = os.getenv("SGLANG_AR_TIMING", "0") == "1"
_AR_TIMING_SYNC = os.getenv("SGLANG_AR_TIMING_SYNC", "1") == "1"
_AR_TIMING_INTERVAL = int(os.getenv("SGLANG_AR_TIMING_INTERVAL", "100"))
...
class TpModelWorker(BaseTpWorker):
    ...
    def _ar_timing_sync(self):
        if _AR_TIMING_ENABLED and _AR_TIMING_SYNC and torch.cuda.is_available():
            torch.cuda.synchronize()

    def _record_ar_timing(
        self,
        forward_s: float = 0.0,
        sample_s: float = 0.0,
        batch_size: int = 0,
    ):
        if not _AR_TIMING_ENABLED:
            return
        timing = self._ar_timing
        timing["count"] += 1
        timing["forward"] += forward_s
        timing["sample"] += sample_s
        timing["batch_size"] += batch_size
        if timing["count"] % _AR_TIMING_INTERVAL != 0:
            return
        count = max(timing["count"], 1)
        avg_bs = timing["batch_size"] / count
        forward_ms = timing["forward"] / count * 1000.0
        sample_ms = timing["sample"] / count * 1000.0
        logger.info(
            "[AR_TIMING] steps=%s avg_bs=%.2f forward=%.3fms sample=%.3fms total=%.3fms sync=%s",
            timing["count"],
            avg_bs,
            forward_ms,
            sample_ms,
            forward_ms + sample_ms,
            _AR_TIMING_SYNC,
        )
    ...
    def forward_batch_generation(
        ...
        if self.is_dllm():
            overlap_fn = (
                getattr(model_worker_batch, "_dllm_overlap_fn", None)
                if model_worker_batch is not None
                else None
            )
            return self._forward_batch_generation_dllm(
                forward_batch, overlap_fn=overlap_fn
            )
        ...
        if self.pp_group.is_last_rank:
            ...
            if (
                ...
                def sample_batch_func():
                    ...
                    if ar_timing:
                        self._ar_timing_sync()
                        self._record_ar_timing(
                            forward_s=_t_forward_end - _t_forward_start,
                            sample_s=time.perf_counter() - _t_sample_start,
