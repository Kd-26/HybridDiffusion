...
class Scheduler(
    ...
    def event_loop_normal(self):
        ...
        while True:
            ...
            if batch:
                ...
                # DLLM fast decode loop: after the first decode step, stay in
                # the mixin's tight loop (prepare_for_dllm_decode each step,
                # inline-absorb new requests) until all reqs finish. Skips
                # per-step outer-scheduler overhead.
                if (
                    self.dllm_config is not None
                    and self.dllm_manager.any_staging_reqs()
                    and batch.is_dllm()
                    and getattr(
                        getattr(self.tp_worker, "dllm_algorithm", None),
                        "__class__",
                        type(None),
                    ).__name__
    ...
    def get_internal_state(self, recv_req: GetInternalStateReq):
        ...
        dllm_algorithm = getattr(self.tp_worker, "dllm_algorithm", None)
        if dllm_algorithm is not None:
            ret["dllm_stats"] = dllm_algorithm.get_stats()
