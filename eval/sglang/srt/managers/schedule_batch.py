...
class MultimodalDataItem:
    ...
    @staticmethod
    def is_empty_list(values):
        if values is None:
            return True
        return (
            len([item for item in flatten_nested_list(values) if item is not None]) == 0
        )
...
class Req(ReqDllmMixin):
    ...
    def _cache_commit_len(self) -> int:
        # DLLM generated tokens are only globally reusable after MambaRadixCache
        # has inserted an aligned boundary and updated cache_protected_len.
        # Anything newer remains request-local and is reclaimed as overallocated.
        if getattr(self, "dllm_config", None) is not None:
            return min(
                self.kv_committed_len,
                max(len(self.origin_input_ids), self.cache_protected_len),
            )
    ...
    def reset_for_retract(self):
        ...
        self.mamba_ping_pong_track_buffer_cpu = None
...
class ScheduleBatch(ScheduleBatchDisaggregationDecodeMixin):
    ...
    def _get_mamba_ping_pong_track_buffer_cpu(self, req: Req) -> List[int]:
        cached = getattr(req, "mamba_ping_pong_track_buffer_cpu", None)
    ...
    def _mamba_radix_cache_v2_req_prepare_for_extend(
        ...
        if req.mamba_ping_pong_track_buffer is None:
            track_buffer = self.req_to_token_pool.mamba_pool.alloc(
                self.req_to_token_pool.mamba_ping_pong_track_buffer_size
            )
            if track_buffer is None:
                raise RuntimeError(
                    "Not enough space for mamba ping pong idx, try to increase "
                    "--mamba-full-memory-ratio."
                )
            req.mamba_ping_pong_track_buffer = track_buffer
            req.mamba_ping_pong_track_buffer_cpu = [
                int(x) for x in track_buffer.detach().cpu().tolist()
            ]
            req.mamba_next_track_idx = 0
            self.req_to_token_pool.req_index_to_mamba_ping_pong_track_buffer_mapping[
                req.req_pool_idx
            ] = track_buffer
        elif req.mamba_next_track_idx is None:
            req.mamba_next_track_idx = 0
        track_buffer_cpu = self._get_mamba_ping_pong_track_buffer_cpu(req)
        mamba_track_indices_cpu.append(track_buffer_cpu[req.mamba_next_track_idx])
    ...
    def prepare_for_dllm_decode(self):
        ...
        for i, req in enumerate(self.reqs):
            if getattr(req, '_inline_prefill', False):
                ...
                if origin_remaining <= 0:
                    ...
                    inline_chunk = getattr(req, "_inline_prefill_max_tokens", None)
                    extend_lens[i] = (
                        min(origin_remaining, int(inline_chunk))
                        if inline_chunk is not None
                        else origin_remaining
                    )
                    prefill_flags[i] = True
            else:
                extend_lens[i] = block_size

        _pure_decode = not any(prefill_flags)
        # PackInfer §3.1: sort by kv_committed_len so adjacent requests have
        # similar KV lengths, reducing tile imbalance in flashinfer attention.
        if _pure_decode and bs > 1:
            self.reqs.sort(key=lambda r: r.kv_committed_len)

        alloc_prefix_lens = [0] * bs
        alloc_seq_lens = [0] * bs
        for i, req in enumerate(self.reqs):
            if getattr(req, "_inline_prefill", False):
                prefix_len = len(req.prefix_indices)
                ...
                prefix_len = req.kv_committed_len
            alloc_prefix_lens[i] = prefix_len
            alloc_seq_lens[i] = prefix_len + extend_lens[i]
        ...
        input_ids_list = [] if not _pure_decode else None
        seq_lens = [0] * bs
        prefix_lens = [0] * bs
        rpx_list = [0] * bs
        attn_mask_types = None if _pure_decode else [DLLM_ATTN_MASK_BIDIR_BLOCK] * bs
        mamba_track_mask_cpu = []
        mamba_track_indices_cpu = []
        mamba_track_seqlens_cpu = []
        dllm_mamba_track_indices_cpu = []
        dllm_mamba_track_steps_cpu = []
        ...
        for i, req in enumerate(self.reqs):
            ...
            if is_prefill:
                ...
                if origin_remaining <= 0:
                    ...
                    # Inline prefill is always a causal pass over the original
            ...
            if get_global_server_args().enable_mamba_extra_buffer():
                if getattr(req, "mamba_ping_pong_track_buffer", None) is None:
                    ...
                    mamba_track_indices_cpu.append(0)
                    mamba_track_seqlens_cpu.append(-1)
                    dllm_mamba_track_indices_cpu.append(0)
                    dllm_mamba_track_steps_cpu.append(-1)
                    dllm_mamba_track_boundaries_cpu.append(-1)
                else:
                    self._mamba_radix_cache_v2_req_prepare_for_extend(
                        req,
                        mamba_track_mask_cpu,
                    ...
                    if next_boundary <= prefix_len + extend_lens[i]:
                        ...
                        if _EXTRA_BUFFER_TRACE:
                            logger.warning(
                                "[HYBRID_DIFFUSION_SELF_SPEC_EXTRA_BUFFER] event=prepare_track "
                                "rid=%s req_pool_idx=%s pure_decode=%s prefill=%s "
                                "prefix_len=%s extend_len=%s kv_committed_len=%s "
                                "next_boundary=%s step=%s next_track_idx=%s "
                                "cache_protected_len=%s phase=%s",
                                ...
                                getattr(req, "mamba_next_track_idx", None),
                        ...
                        if _EXTRA_BUFFER_TRACE:
                            logger.warning(
                                "[HYBRID_DIFFUSION_SELF_SPEC_EXTRA_BUFFER] event=prepare_no_track "
                                "rid=%s req_pool_idx=%s pure_decode=%s prefill=%s "
                                "prefix_len=%s extend_len=%s kv_committed_len=%s "
                                "next_boundary=%s next_track_idx=%s "
                                "cache_protected_len=%s phase=%s",
                                getattr(req, "rid", None),
                                req.req_pool_idx,
                                _pure_decode,
        ...
        # HybridDiffusion self-spec commits a variable number of tokens per round, so total KV
        # length must be recomputed rather than incremented by block_size.
        self.seq_lens_sum = sum(seq_lens)
        self.prefix_lens = prefix_lens
        self.extend_lens = extend_lens
        ...
        if get_global_server_args().enable_mamba_extra_buffer():
            if _pure_decode and not any(mamba_track_mask_cpu):
                ...
                if (
                    ...
                    or _inactive_track_cache[0] != _inactive_key
                ):
                    self._dllm_inactive_track_cache = (
                        _inactive_key,
                        torch.zeros((bs,), dtype=torch.int64, device=self.device),
        ...
        if all(el == block_size for el in extend_lens):
            ...
            if _scatter_cache is not None and _scatter_cache[0] == _cache_key:
                ...
                rpx_t = torch.tensor(rpx_list, dtype=torch.long, device=self.device)
        ...
        self._dllm_attn_mask_types_cpu = attn_mask_types
        self.dllm_mamba_track_indices_cpu = dllm_mamba_track_indices_cpu
        self.dllm_mamba_track_steps_cpu = dllm_mamba_track_steps_cpu
        self.dllm_mamba_track_boundaries_cpu = dllm_mamba_track_boundaries_cpu
    ...
    def get_model_worker_batch(
        ...
        dllm_attn_mask_types_cpu = getattr(self, "_dllm_attn_mask_types_cpu", None)
        if dllm_attn_mask_types_cpu is None and self.dllm_config is not None:
            has_prefill_req = any(req.is_dllm_prefill() for req in self.reqs)
            if has_prefill_req:
                dllm_attn_mask_types_cpu = [
                    (
                        DLLM_ATTN_MASK_CAUSAL_PREFILL
        ...
        return ModelWorkerBatch(
            ...
            dllm_attn_mask_types_cpu=dllm_attn_mask_types_cpu,
            dllm_mamba_track_indices_cpu=getattr(
                self, "dllm_mamba_track_indices_cpu", None
            ),
            dllm_mamba_track_steps_cpu=getattr(
                self, "dllm_mamba_track_steps_cpu", None
            ),
