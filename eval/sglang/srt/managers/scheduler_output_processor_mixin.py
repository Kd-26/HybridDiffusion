...
_SCHED_OUTPUT_TIMING = os.getenv("SGLANG_SCHED_OUTPUT_TIMING", "0") == "1"
_SCHED_OUTPUT_TIMING_INTERVAL = int(
    os.getenv("SGLANG_SCHED_OUTPUT_TIMING_INTERVAL", "100")
)
_SCHED_OUTPUT_TIMING_STATE = {}

DEFAULT_FORCE_STREAM_INTERVAL = 50


def _record_sched_output_timing(kind: str, elapsed_s: float, batch_size: int) -> None:
    if not _SCHED_OUTPUT_TIMING:
        return
    stats = _SCHED_OUTPUT_TIMING_STATE.setdefault(
        kind, {"count": 0, "elapsed": 0.0, "batch_size": 0}
    )
    stats["count"] += 1
    stats["elapsed"] += elapsed_s
    stats["batch_size"] += batch_size
    if stats["count"] % _SCHED_OUTPUT_TIMING_INTERVAL != 0:
        return
    count = max(stats["count"], 1)
    logger.info(
        "[SCHED_OUTPUT_TIMING] kind=%s steps=%s avg_bs=%.2f elapsed=%.3fms",
        kind,
        stats["count"],
        stats["batch_size"] / count,
        stats["elapsed"] / count * 1000.0,
    )
...
class SchedulerOutputProcessorMixin:
    ...
    def process_batch_result_dllm(
        ...
        for idx in range(batch.batch_size()):
            ...
            if trim_info is not None:
                trim_count = trim_info.get(
                    "logical_trim_count", trim_info["trim_count"]
                )
            ...
            if track_boundary is not None and req.mamba_ping_pong_track_buffer is not None:
                ...
                if _EXTRA_BUFFER_TRACE:
                    logger.warning(
                        "[HYBRID_DIFFUSION_SELF_SPEC_EXTRA_BUFFER] event=scheduler_track_boundary "
                        "rid=%s req_pool_idx=%s boundary=%s output_len=%s "
                        "kv_committed_len=%s cache_protected_len=%s next_track_idx=%s "
                        "phase=%s",
            ...
            if not finished:
                if not getattr(batch, "_dllm_decode_mode", False) or is_inline_pf:
                    if _EXTRA_BUFFER_TRACE and req.is_dllm():
                        logger.warning(
                            "[HYBRID_DIFFUSION_SELF_SPEC_EXTRA_BUFFER] event=cache_unfinished_call "
                            "rid=%s req_pool_idx=%s inline_prefill=%s dllm_decode=%s "
                            "output_len=%s kv_committed_len=%s dllm_kv_valid_len=%s "
                            "cache_protected_len=%s mamba_last_track_seqlen=%s phase=%s",
                            getattr(req, "rid", None),
                            req.req_pool_idx,
                            is_inline_pf,
                            getattr(batch, "_dllm_decode_mode", False),
                            len(req.output_ids),
                            req.kv_committed_len,
        ...
        if _SCHED_OUTPUT_TIMING:
            _record_sched_output_timing(
                "dllm", time.perf_counter() - _t_output_start, batch.batch_size()
            )
    ...
    def process_batch_result_decode(
        ...
        if _SCHED_OUTPUT_TIMING:
            _record_sched_output_timing(
                "decode", time.perf_counter() - _t_output_start, batch.batch_size()
            )
