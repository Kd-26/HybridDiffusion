...
class ServerArgs:
    ...
    def _handle_missing_default_values(self):
        ...
        if self.mamba_scheduler_strategy == "auto":
            if self._should_use_dllm_mamba_extra_buffer_by_default():
                logger.warning(
                    "Using mamba extra_buffer for diffusion LLM radix cache on a "
                    "hybrid/linear-attention model. Pass --disable-radix-cache "
                    "for the no-buffer baseline."
                )
    ...
    def _should_use_dllm_mamba_extra_buffer_by_default(self) -> bool:
        if self.dllm_algorithm is None or self.disable_radix_cache:
            return False
        ...
        try:
            ...
            logger.warning(
                "Failed to inspect model architecture for diffusion LLM radix "
                "cache defaults (%s). Falling back to no_buffer.", e
            )
            return False
        ...
        hybrid_spec = get_linear_attn_spec_by_arch(model_arch)
        return (
            hybrid_spec is not None
            and hybrid_spec.support_mamba_cache
            and hybrid_spec.support_mamba_cache_extra_buffer
        )
    ...
    def _handle_dllm_inference(self):
        ...
        if config.variant == SelfSpecVariant.HYBRID_DIFFUSION_SHIFT and not self.disable_radix_cache:
            logger.warning(
                "Radix cache is disabled for HybridDiffusion shifted diffusion sampling. "
                "The algorithm needs a prompt prefill forward to capture the "
                "first decode seed, and generated-prefix Mamba tracking for "
                "radix reuse is not implemented yet."
            )
            self.disable_radix_cache = True
            if self.enable_mamba_extra_buffer():
                logger.warning(
                    "Mamba extra_buffer is disabled together with radix cache "
                    "for HybridDiffusion shifted diffusion sampling."
                )
                self.mamba_scheduler_strategy = "no_buffer"
        ...
        if not self.disable_radix_cache:
            if self.enable_mamba_extra_buffer():
                if self.page_size == 1:
                    logger.warning(
                        "Setting page size to FLA_CHUNK_SIZE=%s for diffusion LLM "
                        "radix cache with mamba extra_buffer. page_size=1 remains "
                        "a correctness baseline, not the high-performance radix path.",
                        FLA_CHUNK_SIZE,
                    )
                    self.page_size = FLA_CHUNK_SIZE
                if (
                    self.mamba_track_interval % self.page_size != 0
                    or max(FLA_CHUNK_SIZE, self.page_size)
                    % min(FLA_CHUNK_SIZE, self.page_size)
                    != 0
                ):
                    raise ValueError(
                        "Diffusion LLM radix cache with mamba extra_buffer requires "
                        "page_size to be compatible with mamba_track_interval and "
                        f"FLA_CHUNK_SIZE. Got page_size={self.page_size}, "
                        f"block_size={config.block_size}, "
                        f"mamba_track_interval={self.mamba_track_interval}, "
                        f"FLA_CHUNK_SIZE={FLA_CHUNK_SIZE}."
                    )
            elif self.page_size != 1 and self.page_size % config.block_size != 0:
                logger.warning(
