...
def attn_backend_wrapper(runner: "ModelRunner", full_attn_backend: "AttentionBackend"):
    ...
    if cfg := runner.mambaish_config:
        ...
        if runner.hybrid_gdn_config is not None:
            ...
            gdn_backend = GDNAttnBackend(runner)
            if runner.server_args.dllm_algorithm is not None:
                from sglang.srt.layers.attention.linear.gdn_dllm_backend import (
                    GDNDllmBackend,
                )
                logger.info("Wrapping GDN backend with dLLM support.")
                linear_attn_backend = GDNDllmBackend(gdn_backend, cfg)
            else:
                linear_attn_backend = gdn_backend
