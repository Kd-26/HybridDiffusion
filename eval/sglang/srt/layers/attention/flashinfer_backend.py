...
def _build_dllm_bidir_block_mask(
    ...
    mask = torch.zeros((block_size, block_size), dtype=torch.bool, device=device)
    rows = torch.arange(gen_block_size, device=device).unsqueeze(1)
    cols = torch.arange(block_size, device=device).unsqueeze(0)
    mask[:gen_block_size, :] = cols <= rows
    mask[gen_block_size:, :] = True
    return mask


def _build_dllm_paged_custom_mask(
...
class FlashInferAttnBackend(AttentionBackend):
    ...
    def __init__(
        ...
        # Shared HybridDiffusion/dLLM metadata for the paired native FlashInfer planner.
        self.dllm_config = DllmConfig.from_server_args(model_runner.server_args)
        self.is_dllm_model = self.dllm_config is not None
        # Keep upstream SDAR/LLaDA on their native ragged, non-causal path.
        # The paged two-stream planner is specific to HybridDiffusion algorithms.
        self.is_hybrid_diffusion_algorithm = (
            self.dllm_config is not None
            and self.dllm_config.algorithm
            in ("HybridDiffusionSelfSpec", "LowConfidenceShiftHybridDiffusion")
        )
        self.dllm_causal_prefill = (
            self.dllm_config.causal_prefill if self.dllm_config is not None else False
        )
        self.dllm_uses_bidir_mask = (
            self.dllm_config is not None
            and self.dllm_config.variant
            in (
        ...
        self.dllm_native_bidir_mask_available = (
            is_flashinfer_available()
            and "dllm_bidir_block_mask"
            in inspect.signature(BatchPrefillWithPagedKVCacheWrapper.plan).parameters
            and "dllm_mask_type"
            in inspect.signature(BatchPrefillWithPagedKVCacheWrapper.plan).parameters
        )
        self.dllm_allow_bidir_custom_mask_fallback = (
            os.getenv("SGLANG_DLLM_ALLOW_BIDIR_CUSTOM_MASK_FALLBACK", "0") == "1"
        )
        self.dllm_bidir_block_mask = None
        self.dllm_batched_bidir_masks = {}
    ...
    def _get_dllm_bidir_block_mask(self, device: torch.device) -> Optional[torch.Tensor]:
        if not self.dllm_uses_bidir_mask:
            return None
        if (
            self.dllm_bidir_block_mask is None
            or self.dllm_bidir_block_mask.device != device
        ):
            self.dllm_bidir_block_mask = _build_dllm_bidir_block_mask(
                self.dllm_config.block_size,
                self.dllm_config.algorithm_config.get("gen_block_size", 3),
                device,
            )
        return self.dllm_bidir_block_mask

    def _get_dllm_batched_bidir_mask(
        self, bidir_mask: torch.Tensor, bs: int, device: torch.device
    ) -> torch.Tensor:
        if bs <= 1:
            return bidir_mask.to(device=device, dtype=torch.bool, non_blocking=True)

        bidir_mask = bidir_mask.to(device=device, dtype=torch.bool, non_blocking=True)
        key = (bs, bidir_mask.shape[0], bidir_mask.device)
        cached = self.dllm_batched_bidir_masks.get(key)
        if cached is None or cached.device != device:
            blk = bidir_mask.shape[0]
            cached = torch.zeros((bs * blk, bs * blk), dtype=torch.bool, device=device)
            rows = torch.arange(bs, device=device)
            cached.view(bs, blk, bs, blk)[rows, :, rows, :] = bidir_mask
            self.dllm_batched_bidir_masks[key] = cached
        return cached
    ...
    def init_forward_metadata(self, forward_batch: ForwardBatch):
        if forward_batch.forward_mode.is_decode_or_idle():
            ...
            and (
                getattr(forward_batch, "dllm_attn_mask_types_cpu", None) is None
                or any(
            ...
            if self.dllm_causal_prefill and forward_batch.forward_mode.is_dllm_mode():
    ...
    def init_forward_metadata_capture_cuda_graph(
        ...
        if forward_mode.is_decode_or_idle():
            ...
            prefill_wrappers = []
            dllm_bidir_custom_mask = (
                self._get_dllm_bidir_block_mask(seq_lens.device)
                if self.is_hybrid_diffusion_algorithm
                else None
            )
            use_dllm_native_bidir_mask = (
                self.is_hybrid_diffusion_algorithm
            ...
            self.indices_updater_prefill.update(
                ...
                prefill_wrappers=prefill_wrappers,
                use_ragged=not self.is_hybrid_diffusion_algorithm,
                encoder_lens=encoder_lens,
                spec_info=None,
                **planner_kwargs,
            )
            ...
            self.forward_metadata = PrefillMetadata(
                prefill_wrappers,
                not self.is_hybrid_diffusion_algorithm,
                False,
        ...
        if not self.forward_metadata.use_ragged:
            ...
            if self.forward_metadata.extend_no_prefix:
                ...
                if not self.is_dllm_model:
                    ...
                    causal = True
                elif self.forward_metadata.dllm_force_bidir_mask:
                    causal = False
                ...
                if (
                    ...
                    bidir_mask = self.forward_metadata.dllm_bidir_custom_mask
                    num_qo = q.shape[0]
                    bs = num_qo // bidir_mask.shape[0] if bidir_mask.shape[0] > 0 else 1
                    blk = bidir_mask.shape[0]
                    batch_mask = self._get_dllm_batched_bidir_mask(
                        bidir_mask, bs, q.device
                    )
                    ...
                    self.prefill_wrapper_ragged.begin_forward(
                        ...
                        layer.head_dim,
                        q_data_type=q.dtype,
                        custom_mask=batch_mask.to(q.device),
                    )
...
class FlashInferIndicesUpdaterPrefill:
    ...
    def update_single_wrapper(
        ...
        self.call_begin_forward(
            ...
            fixed_split_size=fixed_split_size,
            multi_item_params=multi_item_params,
            custom_mask_override=custom_mask,
            dllm_native_bidir_mask=dllm_native_bidir_mask,
            dllm_attn_mask_types=dllm_attn_mask_types,
        )
    ...
    def call_begin_forward(
        ...
        fixed_split_size: Optional[int] = None,
        multi_item_params: Optional[MultiItemScoringParams] = None,
        custom_mask_override: Optional[torch.Tensor] = None,
        dllm_native_bidir_mask: bool = False,
        dllm_attn_mask_types: Optional[torch.Tensor] = None,
    ):
        bs = len(seq_lens)
        if spec_info is None:
            ...
            custom_mask = custom_mask_override
        else:
        ...
        dllm_native_kwargs = (
            {
                "dllm_bidir_block_mask": True,
                "dllm_mask_type": dllm_attn_mask_types,
                "dllm_gen_block_size": self.attn_backend.dllm_config.algorithm_config.get(
                    "gen_block_size", 3
                ),
                "dllm_block_size": self.attn_backend.dllm_config.block_size,
            }
            if dllm_native_bidir_mask
            else {}
        )
