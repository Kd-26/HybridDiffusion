...
_DLLM_TOPK_DENSE_MODE = os.getenv("SGLANG_DLLM_TOPK_DENSE", "auto")
_DLLM_TOPK_DENSE_MAX_ROWS = int(os.getenv("SGLANG_DLLM_TOPK_DENSE_MAX_ROWS", "64"))

_is_npu = is_npu()


@triton.jit
def _dllm_topk_probs_kernel(
    vals_ptr,
    probs_ptr,
    n_rows: tl.constexpr,
    top_k: tl.constexpr,
    stride_vals_row: tl.constexpr,
    stride_probs_row: tl.constexpr,
    ...
    mask = offs < top_k
    vals = tl.load(
        vals_ptr + row * stride_vals_row + offs,
        mask=mask,
        other=-float("inf"),
    ).to(tl.float32)
    if temperature != 1.0:
    ...
    if top_p < 1.0:
        ...
        filtered = tl.where(keep & mask, probs, 0.0)
        filtered_denom = tl.sum(filtered, axis=0)
        probs = tl.where(filtered_denom > 0, filtered / filtered_denom, probs)
    tl.store(probs_ptr + row * stride_probs_row + offs, probs, mask=mask)


def _dllm_topk_probs_fused(
    vals: torch.Tensor,
    top_p: float,
    temperature: float,
) -> torch.Tensor:
    if vals.ndim != 2 or vals.stride(-1) != 1:
        raise ValueError("top-k values must be a contiguous 2D tensor")
    n_rows, top_k = vals.shape
    probs = torch.empty_like(vals, dtype=torch.float32)
    block_k = triton.next_power_of_2(top_k)
    _dllm_topk_probs_kernel[(n_rows,)](
        vals,
        probs,
        n_rows,
        top_k,
...
class LogitsProcessorOutput:
    ...
    full_argmax: Optional[torch.Tensor] = None
    full_topk_ids: Optional[torch.Tensor] = None
    full_topk_probs: Optional[torch.Tensor] = None
    dllm_logits_mode: Optional[str] = None
...
class LogitsMetadata:
    ...
    dllm_return_argmax_only: bool = False
    dllm_return_topk_probs: bool = False
    dllm_top_k: int = 0
    dllm_top_p: float = 1.0
    dllm_temperature: float = 1.0
    ...
    def from_forward_batch(cls, forward_batch: ForwardBatch):
        ...
        return cls(
            ...
            dllm_return_argmax_only=bool(
                getattr(forward_batch, "dllm_return_argmax_only", False)
            ),
            dllm_return_topk_probs=bool(
                getattr(forward_batch, "dllm_return_topk_probs", False)
            ),
            dllm_top_k=int(getattr(forward_batch, "dllm_top_k", 0)),
            dllm_top_p=float(getattr(forward_batch, "dllm_top_p", 1.0)),
            dllm_temperature=float(getattr(forward_batch, "dllm_temperature", 1.0)),
...
class LogitsProcessor(nn.Module):
    ...
    def _compute_lm_head_argmax_tiled(
        self,
        hidden_states: torch.Tensor,
        lm_head: VocabParallelEmbedding,
    ) -> Optional[torch.Tensor]:
        """Compute per-row argmax without materializing full DLLM logits."""
        ...
        chunk_size = int(os.getenv("SGLANG_DLLM_ARGMAX_CHUNK_SIZE", "32768"))
        chunk_size = max(1, chunk_size)
        compute_states = hidden_states
        compute_weight = weight
        if self.use_fp32_lm_head:
            compute_states = hidden_states.to(torch.float32)
            compute_weight = weight.to(torch.float32)
        else:
            compute_states = hidden_states.to(weight.dtype)

        best_vals = None
        best_ids = None
        vocab = compute_weight.shape[0]
        for start in range(0, vocab, chunk_size):
            end = min(start + chunk_size, vocab)
            logits = torch.matmul(compute_states, compute_weight[start:end].T)
            vals, ids = logits.max(dim=-1)
            ids = ids + start
            if best_vals is None:
                ...
                take = vals > best_vals
                best_vals = torch.where(take, vals, best_vals)
                best_ids = torch.where(take, ids, best_ids)
        return best_ids

    def _compute_lm_head_topk_tiled(
        self,
        hidden_states: torch.Tensor,
        lm_head: VocabParallelEmbedding,
        top_k: int,
        ...
        """Compute sparse top-k/top-p probabilities for DLLM sampling."""
        if top_k <= 0:
            return None
        if self.do_tensor_parallel_all_gather or self.do_tensor_parallel_all_gather_dp_attn:
            return None
        ...
        weight = lm_head.weight
        k = min(top_k, weight.shape[0])
        chunk_size = int(os.getenv("SGLANG_DLLM_TOPK_CHUNK_SIZE", "32768"))
        chunk_size = max(k, chunk_size)
        ...
        for start in range(0, vocab, chunk_size):
            ...
            logits = torch.matmul(compute_states, compute_weight[start:end].T)
            vals, ids = logits.topk(min(k, end - start), dim=-1)
            ids = ids + start
            if best_vals is None:
                best_vals = vals
                best_ids = ids
                ...
                best_vals, take = merged_vals.topk(k, dim=-1)
                best_ids = merged_ids.gather(1, take)

        if self.logit_scale is not None:
            best_vals = best_vals * self.logit_scale
        if self.final_logit_softcapping:
            best_vals = self.final_logit_softcapping * torch.tanh(
                best_vals / self.final_logit_softcapping
            )
        probs = _dllm_topk_probs_fused(best_vals.contiguous(), top_p, temperature)
        return best_ids, probs

    def _compute_lm_head_topk_dense(
        self,
        hidden_states: torch.Tensor,
        lm_head: VocabParallelEmbedding,
        logits_metadata: LogitsMetadata,
        ...
        """Dense matmul + top-k for small DLLM block rows.

        The tiled path minimizes memory but launches multiple matmul/top-k rounds.
        For block-7 C=8, one dense LM-head matmul can be faster and still avoids
        retaining full logits after extracting the compact top-k distribution.
        """
        if top_k <= 0:
            return None
        if _DLLM_TOPK_DENSE_MODE == "0":
            return None
        if _DLLM_TOPK_DENSE_MODE == "auto" and hidden_states.shape[0] > _DLLM_TOPK_DENSE_MAX_ROWS:
            return None

        logits = self._get_logits(hidden_states, lm_head, logits_metadata)
        k = min(top_k, logits.shape[-1])
        vals, ids = logits.topk(k, dim=-1)
        probs = _dllm_topk_probs_fused(vals.contiguous(), top_p, temperature)
        del logits
        return ids, probs
    ...
    def _get_dllm_logits(
        ...
        if logits_metadata.dllm_return_argmax_only:
            full_argmax = self._compute_lm_head_argmax_tiled(
            ...
            if full_argmax is not None:
                return LogitsProcessorOutput(
                    ...
                    dllm_logits_mode="argmax",
                )
        if logits_metadata.dllm_return_topk_probs:
            sparse = self._compute_lm_head_topk_dense(
                hidden_states,
                lm_head,
                logits_metadata,
                logits_metadata.dllm_top_k,
                logits_metadata.dllm_top_p,
                logits_metadata.dllm_temperature,
            )
            sparse_mode = "topk_dense"
            if sparse is None:
                sparse = self._compute_lm_head_topk_tiled(
                    hidden_states,
                    lm_head,
                    logits_metadata.dllm_top_k,
                    logits_metadata.dllm_top_p,
                ...
                sparse_mode = "topk_tiled"
            if sparse is not None:
                ids, probs = sparse
                return LogitsProcessorOutput(
                    full_logits=None,
                    full_topk_ids=ids,
                    full_topk_probs=probs,
                    next_token_logits=None,
                    dllm_logits_mode=sparse_mode,
                )
            if os.getenv("SGLANG_DLLM_LOG_SPARSE", "0") == "1":
                logger.info(
                    "[DLLM sparse logits] topk fallback rows=%s top_k=%s "
                    "tp_all_gather=%s dp_attn=%s has_weight=%s logit_scale=%s "
                    "softcap=%s",
        ...
        elif os.getenv("SGLANG_DLLM_LOG_SPARSE", "0") == "1":
            logger.info(
                "[DLLM sparse logits] not requested rows=%s argmax_only=%s "
                "top_k=%s temp=%s",
                hidden_states.shape[0],
                logits_metadata.dllm_return_argmax_only,
                logits_metadata.dllm_top_k,
                logits_metadata.dllm_temperature,
            )
